...
async function rankBeachesForTime(speciesId, predictionTimestamp) {
  // Temporary timing instrumentation — visible in the UI (not console.log,
  // since this needs to be readable on a phone with no devtools access)
  // so we can see exactly which beach or stage is actually slow instead
  // of guessing at a fix again. Purely additive: existing callers
  // (rankBeachesByClarity) ignore the new `timings` field entirely.
  const overallStart = performance.now();
  const settled = await Promise.all(BEACHES.map(async (beach) => {
    const beachStart = performance.now();
    let envelope;
    try { envelope = await predictAsOfTimestamp(beach, speciesId, predictionTimestamp, browserDataAccess); }
    catch (err) { return { beach, available: false, reason: "Prediction failed for this beach/time.", ms: Math.round(performance.now() - beachStart), errMsg: err?.message }; }
    const ms = Math.round(performance.now() - beachStart);
    if (!envelope.result) return { beach, available: false, reason: "Insufficient data (e.g. no tide coverage) at this time.", ms }; // never fabricate a score — matches predictAsOfTimestamp's own contract
    return { beach, available: true, envelope, ms };
  }));
  const overallMs = Math.round(performance.now() - overallStart);
  const timings = settled.map((r) => ({ beachName: r.beach.name, ms: r.ms, errMsg: r.errMsg })).sort((a, b) => b.ms - a.ms);
  ...
  return { speciesId, predictionTimestamp, beaches, bestBeach: beaches[0] || null, unavailableBeaches, timings, overallMs };
}
...
function BestBeachComparatorScreen({ onSelectBeach }) {
  ...
  return (
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      {/* Temporary diagnostic — remove once we've actually found the
          bottleneck. Shows real per-beach timing so we can see exactly
          what's slow instead of guessing at another fix. */}
      {!ranking.loading && ranking.overallMs != null && (
        <div style={{ background: "#1A0F0F", border: "1px solid #4A2020", borderRadius: 8, padding: 10, marginBottom: 10 }}>
          <div style={{ color: "#FF8A8A", fontSize: 11, fontWeight: 700, marginBottom: 4 }}>DEBUG: total {ranking.overallMs}ms for {BEACHES.length} beaches</div>
          {(ranking.timings || []).slice(0, 6).map((t, i) => (
            <div key={i} style={{ color: "#C99", fontSize: 10.5 }}>{t.beachName}: {t.ms}ms{t.errMsg ? ` — ERROR: ${t.errMsg}` : ""}</div>
          ))}
        </div>
      )}
