...
async function predictAsOfTimestamp(beach, speciesId, predictionTimestamp) {
  ...
  const baitInfo = inferBaitActivity(buoyReading, predictionTimestamp);
...
function buildSnapshotPayload(envelope) {
  ...
  return {
    ...
    bait_tier: environmentalInputs.baitTier, bait_level: environmentalInputs.baitLevel, bait_source: sourceMeta.buoy,
...
const MODEL_VERSION = "BFR_MODEL_V10_1";
...
// ===========================================================================
// PREDICTION SNAPSHOTS (Priority 5) — a standalone, append-only record of
// "what the model predicted," fully decoupled from trip logging. A trip
// observation (sighted/bit/caught) and a prediction snapshot are different
// concepts: every scored species produces a snapshot, whether or not
// anything was ever observed. Self-contained (no foreign key back to a
// trips row) so a future bulk backtest — calling predictAsOfTimestamp()
// hundreds of times with no trip-log UI involved at all — can persist
// snapshots the same way. Every write here is a pure INSERT (see the
// Worker route); there is no UPDATE path, which is what makes a saved
// snapshot immutable by construction, not by convention.
//
// Three timestamps, kept explicit and never collapsed into each other:
//   predictionTimestamp — what moment this prediction represents
//   retrievedAt          — when this snapshot was actually written (now)
//   (observationTimestamp belongs to trip_observations, not here — a
//    snapshot is a prediction record, not an observation record)
// ===========================================================================

// Builds one snapshot object matching the prediction_snapshots schema
// exactly, from a scoreSpecies() result (or predictAsOfTimestamp()'s
// .result) plus the shared environmental context it was computed from.
// Pure — no network call, no side effect.
function buildPredictionSnapshot(beach, speciesId, result, position, env, predictionTimestamp) {
  ...
  return {
    ...
    feeding: result.feeding ?? null,
    access: result.access ?? null,
    final_score: result.score ?? null,
    confidence: result.confidence ?? null,
    predicted_zone: position?.primaryZone ?? null,
    predicted_distance_min: position?.distanceYd?.[0] ?? null,
    predicted_distance_max: position?.distanceYd?.[1] ?? null,
    wave_ft: env?.waveFt ?? null, wave_ft_source: env?.waveSource ?? null, wave_period_s: env?.wavePeriodS ?? null,
    wind_kt: env?.windKt ?? null, wind_kt_source: env?.windSource ?? null, wind_dir_deg: env?.windDirDeg ?? null,
    water_temp_f: env?.waterTempF ?? null, water_temp_f_source: env?.waterTempSource ?? null,
    clarity_score: env?.clarityScore ?? null, clarity_label: env?.clarityLabel ?? null, clarity_source: env?.claritySource ?? null,
    tide_direction: env?.tideDirection ?? null, tide_flow_pct: env?.tideFlowPct ?? null, tide_source: env?.tideSource ?? null,
    bait_tier: env?.baitTier ?? null, bait_level: env?.baitLevel ?? null, bait_source: env?.baitSource ?? null,
    moon_phase_name: env?.moonPhaseName ?? null, moon_illumination_pct: env?.moonIllumPct ?? null, moon_source: env?.moonSource ?? null,
    major_positive_factors: positiveFactors.length ? JSON.stringify(positiveFactors) : null,
    major_limiting_factors: limitingFactors.length ? JSON.stringify(limitingFactors) : null,
  };
}

// Persists an array of snapshot objects in one call. Each call always
// creates new rows — there is no update-by-id path, so recomputing the
// same prediction under a newer MODEL_VERSION simply produces additional
// rows; the old ones are never touched.
async function savePredictionSnapshots(snapshots) {
  const valid = (snapshots || []).filter(Boolean);
  if (valid.length === 0) return { ok: true, count: 0 };
  return postToProxy("/predictions/log", { snapshots: valid });
}
            ...
            {showMoreConditions && (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #16232E" }}>
                  ...
                  <span style={{ color: "#7590A0", fontSize: 12.5, flex: 1 }}>Bait {baitInfo && baitInfo.tier !== "unknown" && <span style={{ color: "#F5A623", fontSize: 8.5 }}> ● ESTIMATED</span>}</span>
                  <span style={{ color: "#DCE8EE", fontSize: 12.5 }}>{baitInfo && baitInfo.tier !== "unknown" ? baitInfo.level : "Unknown"}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #16232E" }}>
