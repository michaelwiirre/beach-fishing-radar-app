...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  const {
    hasBuoy, noaaState, buoyState, tideStage, moonFactor, buoyFactor,
    liveResults, loadNoaa, loadBuoy,
      ...
      {tab === "overview" ? (
        <div style={{ padding: "8px 14px" }}>
          ...
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: "2px 12px", marginBottom: 4 }}>
            ...
            <ConditionRow icon={Eye} label="Clarity" value="Not connected yet" />
        ...
        <div style={{ padding: "8px 14px" }}>
          ...
          {liveEntries.length > 0 && (
            <div style={{ color: "#4A6270", fontSize: 10.5, marginBottom: 10, lineHeight: 1.4 }}>
              All live scores are built from tide, moon, and (where the buoy reaches) wave/pressure. Water clarity and bait presence are still not connected \u2014 see the Activity tab for real public reports instead.
            </div>
          )}
        </div>
      ) : (
    ...
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      <div style={{ marginTop: 16, background: "#1A1408", border: "1px solid #6B3B1E", borderRadius: 10, padding: 12 }}>
        ...
        <div style={{ color: "#E0C6A8", fontSize: 12, lineHeight: 1.5 }}>
          Water clarity has no live source. Recent reports are live only for the 2 beaches with a confirmed RSS feed (Activity tab). Social-media ingestion still needs a per-platform ToS review before anything is built there.
        </div>
      </div>
    </div>
  );
