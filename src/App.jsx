...
function useLiveSpeciesScores(beachId) {
  ...
  // When the buoy reports wind but no wave height (some stations genuinely
  // lack a wave sensor), fall back to a wind-derived estimate rather than
  // silently dropping the wave axis from every downstream calculation.
  // waveHeightIsEstimated marks this so the UI never shows it as LIVE.
  const effectiveBuoy = (() => {
      ...
      {tab === "overview" ? (
        <div style={{ padding: "8px 14px" }}>
          ...
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: "2px 12px", marginBottom: 4 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: "1px solid #16232E" }}>
              <Waves size={14} color={effectiveBuoy?.waveHeightFt != null ? (effectiveBuoy.waveHeightIsEstimated ? "#F5A623" : "#17D9C4") : "#5A7A8A"} />
              <span style={{ color: "#7590A0", fontSize: 12.5, flex: 1 }}>
                Swell (offshore) {effectiveBuoy?.waveHeightFt != null && (effectiveBuoy.waveHeightIsEstimated
            ...
            {hasBuoy
              ? buoyState.loading
                ...
                : buoyState.fatalError
                  ...
                  : `Wind/swell/temp/pressure from NDBC buoy ${NDBC_STATIONS[beach.id].stationId} (${NDBC_STATIONS[beach.id].stationName}) — an offshore reading, not measured at the beach itself.${effectiveBuoy?.waveHeightIsEstimated ? " This station has no wave sensor, so swell height is ESTIMATED from wind speed (Beaufort scale), not measured." : ""} Water clarity is estimated from this data, not directly measured.`
