...
function LbsfScreen() {
  ...
  const nowScore = (() => {
    ...
    return lbsfScoreRow(row, conditions.data.waterTempF, mulletRun);
  })();

  const nightsScored = (forecast.data?.nights || []).map((n) => {
    const row = { ts: new Date(n.date + "T21:30:00"), wind_mph: lbsfWindToNum(n.windSpeed), tide_direction: n.tideDirection, forecast_text: n.shortForecast || "" };
    const scored = lbsfScoreRow(row, n.waterTempEstF, mulletRun);
    return { ...n, ...scored };
  });
  ...
  return (
    <div style={{ padding: "14px 14px 100px" }}>
      ...
      {forecast.loading ? (
        ...
        nightsScored.map((n) => (
          <div key={n.date} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 8, background: "#0E1B26", border: "1px solid #1F3444", marginBottom: 4 }}>
            <div>
              <div style={{ color: "#DCE8EE", fontSize: 12.5 }}>{lbsfFmtNight(n.date)}</div>
              <div style={{ color: "#5A7A8A", fontSize: 10.5 }}>{n.species}{n.isEstimate ? " · water temp estimated" : ""}</div>
            </div>
