...
function computeAccess(speciesId, position, windKt, flowStrength) {
  ...
  if (access < 90) {
    ...
    explanation = parts.length ? `Catchability reduced — ${parts.join("; ")}.` : null;
  }
...
function scoreSpecies(species, tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo) {
  ...
  const position = predictFishPosition(tideStage, buoy, SPECIES_RULES[species]?.zoneBias || 0);
  ...
  if (accessResult) {
    factors.push({ label: accessResult.explanation || "Fish are within comfortable casting range", shortLabel: "Access", delta: accessResult.access - 100 });
  }
