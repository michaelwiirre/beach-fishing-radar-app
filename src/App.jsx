...
async function predictAsOfTimestamp(beach, speciesId, predictionTimestamp) {
  ...
  // Priority 10 — a genuinely future timestamp takes forecast inputs; a
  // past or current one keeps the existing historical-observation path,
  // completely unchanged. The threshold is real "now" at call time, not a
  // fixed cutoff, so this correctly reclassifies as time passes.
  const isFuture = predictionTimestamp.getTime() > Date.now();
...
const MODEL_VERSION = "BFR_MODEL_V10_3";
...
// contains the moment or it doesn't.
function findGridpointValueAt(layerValues, targetDate) {
  ...
  for (const entry of layerValues) {
    ...
    if (targetMs >= startMs && targetMs < startMs + durMs) return { value: entry.value, validTime: entry.validTime };
  }
  return null;
}
...
function resolveForecastInputs(forecastBundle, predictionTimestamp) {
  ...
  const get = (field) => {
    ...
    return { value: convertForecastValue(found.value, layer.uom), status: "forecast", validTime: found.validTime, source };
  };
  return {
    ...
    precipProbabilityPct: get("probabilityOfPrecipitation"), precipAmountIn: get("quantitativePrecipitation"),
