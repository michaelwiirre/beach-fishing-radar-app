...
function computePresence(species, tempF, month, baitInfo) {
  ...
  let bait = 0;
  if (baitInfo?.tier === "observed") bait = Math.round(10 * r.baitWeight * Math.min(1.5, baitInfo.forageStrength ?? 1));
...
function computeFeeding(species, { tideStage, waveFt, clarityScore, windKt, isNight, lowLight, baitInfo }) {
  ...
  const baitSub = baitInfo?.tier === "observed"
    ? Math.round(50 + 30 * Math.min(1.5, baitInfo.forageStrength ?? 1))
...
// Shared sampling core for predictNextBestBite() and rankFutureSpeciesWindows()
...
// window-ranking logic across time (rankFutureSpeciesWindows). Neither
// introduces a second scoring formula: rankBeaches literally IS
// predictAsOfTimestamp()'s score, sorted; rankFutureSpeciesWindows literally IS
...
async function rankFutureSpeciesWindows({ beachId, speciesId, start, end, maxWindows = 3, stepMinutes = 30 }) {
  ...
  for (let t = start.getTime(); t <= end.getTime(); t += stepMs) {
    ...
    samples.push({ time: ts, score: envelope.result.score, envelope });
  }
  if (samples.length === 0) {
    return { windows: [], error: "No valid predictions across this range — required inputs (at minimum tide) were unavailable throughout." };
  }

  // Same greedy local-maxima extraction as before: take the global peak,
  // expand its window while nearby samples stay within 8 points, remove
  // those samples, repeat.
  ...
  while (remaining.length > 0 && windows.length < maxWindows) {
    ...
    windows.push({
      ...
      peakTime: peak.time, score: peak.score,
  ...
  return {
    ...
    description: "Each window's score is the actual species prediction (Presence/Feeding/Access) evaluated at that moment — forecast wind/wave/precip where available, deterministic tide, existing forage rules. Missing forecast inputs (e.g. water temperature) lower confidence rather than being fabricated.",
  };
}
...
// ===========================================================================
// BEACH SHORELINE ORIENTATION (Priority 11 Part 3) — computed geometrically
// from the real coordinates already above, not hand-authored per beach and
// not one fixed compass assumption for the whole coast. Beaches are sorted
// by actual latitude (NOT the authored array order, which is "north to
// south" in spirit but not strictly latitude-monotonic everywhere — e.g.
// Playalinda sits between Flagler and New Smyrna in the array despite
// being geographically south of both) so that "northern/southern
// neighbor" means what it says; using raw array-position neighbors
// produced at least one clearly wrong heading (New Smyrna came out around
// 239°, roughly backwards) before this was fixed. The bearing between a
// beach's true nearest-latitude neighbors approximates the local
// coastline direction; rotating that -90° gives the seaward-normal
// (onshore) direction — valid specifically because Florida's Atlantic
// coast has the ocean consistently on the east side of a roughly
// north-south coastline. This is a reasonable heading, not surveyed
// precision, and is stored explicitly per beach (BEACH_SEAWARD_NORMAL) so
// it's auditable — see the Priority 11 report for the full list. The
// tightly clustered Juno/Jupiter/Palm Beach points still rely on very
// short baselines even after the sort fix, simply because they're
// genuinely close together — expected, not a bug.
...
const BEACH_SEAWARD_NORMAL = (() => {
  // Sorted by actual latitude, not array order — the authored BEACHES
  // array is "north to south" in spirit but not strictly latitude-
  // monotonic everywhere (e.g. Playalinda sits between Flagler and New
  // Smyrna in the array despite being geographically south of both,
  // and the Juno/Jupiter/Palm Beach cluster follows a driving-route
  // order rather than strict latitude). Using array-position neighbors
  // directly produced at least one clearly wrong heading (New Smyrna came
  // out around 239°, roughly backwards) before this fix — sorting by real
  // latitude first makes "northern/southern neighbor" mean what it says.
  const byLat = [...BEACHES].sort((a, b) => b.lat - a.lat);
  const result = {};
  for (let i = 0; i < byLat.length; i++) {
    const prev = byLat[Math.max(0, i - 1)];
    const next = byLat[Math.min(byLat.length - 1, i + 1)];
    const coastlineBearing = bearingBetweenPoints(prev.lat, prev.lon, next.lat, next.lon);
    result[byLat[i].id] = Math.round((((coastlineBearing - 90) % 360) + 360) % 360);
  }
  return result;
})();
