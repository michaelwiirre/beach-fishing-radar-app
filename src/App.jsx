    ...
    if (ndbcStation) {
      try {
        ...
        const [todayRows, prevRows] = await Promise.all([
          fetchBuoyHistoryDay(ndbcStation, nowCompact),
          fetchBuoyHistoryDay(ndbcStation, prevCompact),
        ]);
...
async function rankFutureSpeciesWindows({ beachId, speciesId, start, end, maxWindows = 3, stepMinutes = 30 }) {
  ...
  const windows = merged.map((w) => {
    ...
    return {
      ...
      debugStartIdx: w.startIdx, debugEndIdx: w.endIdx, debugPeakIdx: w.peakIdx, // TEMPORARY — remove once the zero-width-window issue is resolved
    };
  });
  ...
  return {
    ...
    debugSampleCount: samples.length, // TEMPORARY — remove once the zero-width-window issue is resolved
  };
}
...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  // Defensive display guarantee: whatever upstream mechanism might produce
  // an out-of-order pair, this never lets the UI show an end time before
  // a start time. Does not change any score, ranking, or which window is
  // selected — display-formatting only.
  const fmtWindowRange = (start, end) => {
    const [a, b] = start.getTime() <= end.getTime() ? [start, end] : [end, start];
    return `${fmtWindowTime(a)}–${fmtWindowTime(b)}`;
  };
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                {bestBiteState.loading ? (
                  ...
                  <div style={{ marginTop: 4 }}>
                    ...
                    {/* TEMPORARY diagnostic — remove once the zero-width-window issue is resolved */}
                    <div style={{ color: "#F5A623", fontSize: 9, marginTop: 3, fontFamily: "monospace" }}>
                      DEBUG samples={bestBiteState.result?.debugSampleCount} start={bestWindow.debugStartIdx} end={bestWindow.debugEndIdx} peak={bestWindow.debugPeakIdx}
                    </div>
                    {secondaryWindow && (
...
async function rankBeachesForTime(speciesId, predictionTimestamp) {
  const settled = await Promise.all(BEACHES.map(async (beach) => {
    let envelope;
    try { envelope = await predictAsOfTimestamp(beach, speciesId, predictionTimestamp); }
      ...
      {ranking.loading ? (
        ...
        <>
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: 14, marginBottom: 14 }}>
            ...
            <div onClick={() => onSelectBeach(winner.beachId)} style={{ cursor: "pointer" }}>
              <div style={{ color: "#E7EFF3", fontSize: 16, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>
                #1 {winner.beachName} — {winner.finalScore}
              </div>
              <div style={{ color: "#8AA6B8", fontSize: 11, marginTop: 4 }}>
                Presence: {winner.presence} · Feeding: {winner.feeding} · Access: {winner.access} · {winner.confidence} confidence
              </div>
            </div>
            {runnerUp && (
          ...
          {ranking.beaches.slice(0, 8).map((b, i) => {
            ...
            return (
              <div key={b.beachId} style={{ marginBottom: 4, borderRadius: 8, background: i === 0 ? "#0F2B28" : "#0E1B26", border: "1px solid #1F3444", overflow: "hidden" }}>
                ...
                {isExpanded && (
                  <div style={{ padding: "0 10px 10px" }}>
                    <div style={{ color: "#4A6270", fontSize: 9.5, marginBottom: 4 }}>
                      {b.confidence} confidence · {b.isFuture ? `${hourOptions[selectedHourIdx].label} forecast` : "current observations"}
                    </div>
                    <div style={{ color: "#4A6270", fontSize: 9.5, marginBottom: 6 }}>
