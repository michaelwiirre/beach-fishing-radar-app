...
async function predictAsOfTimestamp(beach, speciesId, predictionTimestamp) {
  ...
  if (noaaStation) {
    try {
      ...
      if (tideRows && tideRows.length > 0) sourceMeta.tide = "historical";
    } catch {
  ...
  const clarityEstimate = estimateWaterClarity(buoyReading, precipReading, tideStage);
  ...
  // including the predictionTimestamp propagation from Priority 2.
...
function buildSnapshotPayload(envelope) {
  const { beachId, speciesId, predictionTimestamp, retrievedAt, sourceMeta, environmentalInputs, result } = envelope;
  ...
  return {
    ...
    water_temp_f: environmentalInputs.waterTempF, water_temp_f_source: sourceMeta.buoy,
    clarity_score: environmentalInputs.clarityScore, clarity_label: environmentalInputs.clarityLabel, clarity_source: sourceMeta.buoy,
...
const MODEL_VERSION = "BFR_MODEL_V10_2";
...
function computePresence(species, tempF, month, baitInfo) {
  ...
  // dominantType (Priority 9) scales the SAME baitWeight toward the
  // species' affinity for whichever forage was actually observed — not a
  // second weight. Absent dominantType (the case for all seasonal/unknown
  // bait, i.e. current behavior), this is identical to before.
  const affinityScale = baitInfo?.dominantType ? baitTypeAffinity(species, baitInfo.dominantType) : 1;
  let bait = 0;
  if (baitInfo?.tier === "observed") bait = Math.round(10 * r.baitWeight * affinityScale);
...
function computeFeeding(species, { tideStage, waveFt, clarityScore, windKt, isNight, lowLight, baitInfo }) {
  ...
  // dominantType (Priority 9) scales how strongly "observed" bait pushes
  // Feeding based on this species' actual affinity for that forage type —
  // a flat 95 for every species regardless of fit was the old (never
  // actually triggered) behavior. Absent dominantType, unchanged.
  const baitSub = baitInfo?.tier === "observed"
    ? (baitInfo.dominantType ? Math.round(50 + 45 * baitTypeAffinity(species, baitInfo.dominantType)) : 95)
...
function resolveForageEvidence(trips, predictionTimestamp) {
  ...
  for (const trip of trips || []) {
    ...
    for (const obs of trip.observations || []) {
      ...
      if (!(obs.sighted || obs.caught)) continue; // only positive sightings are presence evidence
      ...
      forage[obs.subject_id] = {
        presence: obs.caught ? "confirmed" : "observed",
        concentration: obs.count != null ? (obs.count >= 500 ? "dense" : obs.count >= 50 ? "moderate" : "scattered") : "unknown",
        source: "direct_user_trip_log",
        observedAtUtc: obsTime.toISOString(),
        freshnessTier,
        confidence: forageFreshnessConfidence(freshnessTier),
      };
    }
  }
  return forage;
...
// Combines real forage evidence with the existing seasonal-inference
// baitInfo, producing the SAME baitInfo shape every downstream function
// already understands — no new parameter threading required. When no
// sufficiently fresh direct observation exists for any bait type, this
// returns the seasonal baitInfo completely unchanged (100% of current
// behavior preserved when no trip-log bait data exists, which remains
// true for essentially all live traffic today). "observed" tier — already
// present throughout the engine, previously never actually set — is only
// set here, and only when real, timestamped, look-ahead-safe evidence
// supports it.
function buildEnhancedBaitInfo(seasonalBaitInfo, forage, speciesId) {
  let best = null;
  for (const bt of BAIT_TYPES) {
    const ev = forage[bt.id];
    const mult = forageFreshnessMultiplier(ev.freshnessTier);
    if (mult <= 0) continue;
    const affinity = baitTypeAffinity(speciesId, bt.id);
    const weight = mult * affinity * (ev.presence === "confirmed" ? 1.15 : 1.0);
    if (!best || weight > best.weight) best = { bt, ev, weight, mult, affinity };
  }
  if (!best) return seasonalBaitInfo; // no usable direct evidence — unchanged seasonal fallback

  return {
    tier: "observed",
    level: `${best.ev.concentration !== "unknown" ? best.ev.concentration[0].toUpperCase() + best.ev.concentration.slice(1) + " " : ""}${BAIT_TYPES.find((b) => b.id === best.bt.id).name.toLowerCase()} — ${best.ev.freshnessTier.replace("_", " ")}`,
    score: Math.max(0, Math.min(100, Math.round(50 + best.weight * 45))), // same 0-100 scale as the seasonal score it replaces — was previously able to exceed 100 for a confirmed catch of a high-affinity type, fixed here
    basis: `Direct trip-log observation of ${best.bt.name.toLowerCase()} (${best.ev.presence}), ${best.ev.freshnessTier.replace("_", " ")}, ${best.ev.concentration} concentration`,
...
function predictFishPosition(tideStage, buoy, zoneBias, baitInfo) {
  ...
  // during an actual bait push/blitz. Observed bait pulls hardest;
  // inferred-seasonal only nudges. This is the mechanism that will matter
  // most once live bait reporting exists, but it applies right now too.
  let baitPulledIn = false;
  if (baitInfo?.tier === "observed") { idx -= 1.2; baitPulledIn = true; }
...
function scoreSpecies(species, tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo, isLowLight, predictionTimestamp) {
  ...
  const position = predictFishPosition(tideStage, buoy, SPECIES_RULES[species]?.zoneBias || 0, baitInfo);
  const accessResult = computeAccess(species, position, buoy?.windSpeedKt, tideStage.flowStrength);

  const { final, opportunistic } = computeFinalActivity(species, presence ?? 50, feedingResult.score, baitInfo, secondaryAdj, accessResult?.access);
  ...
  return {
    score: final, presence, feeding: feedingResult.score, access: accessResult?.access ?? null,
...
// Shared sampling core for predictNextBestBite() and rankForecastableTideLightWindows()
// (Priority 8) — deliberately NOT the species Presence/Feeding/Access
// engine. Only tide (deterministic NOAA prediction), moon (astronomical
// calculation), and sun timing are genuinely knowable in advance; wave,
// wind, water temp, clarity, and bait have no forecast source in this app
// and are never faked here. speciesId is optional and, when given, applies
// ONLY the species' existing tide-direction preference from SPECIES_RULES
// (the same favoredDirection/directionBonus already used by the live
// scoring engine, reused verbatim — not a new weight) so window ranking
// can be species-aware without pretending to run the full model forward.
...
// ===========================================================================
// RANKING (Priority 8) — "where should I fish, and when?" Aggregates the
// EXISTING prediction engine across beaches (rankBeaches) or the EXISTING
// forecastable-window logic across time (rankForecastableTideLightWindows). Neither
// introduces a second scoring formula: rankBeaches literally IS
// predictAsOfTimestamp()'s score, sorted; rankForecastableTideLightWindows literally IS
// sampleTideWindows() above, which predictNextBestBite already used.
// ===========================================================================
...
// the existing tide-direction preference only) across [start, end]. Finds
// the top N distinct local-maxima windows rather than only the single best
// one predictNextBestBite returns. Returns null outright if this beach has
// no NOAA station mapped or no live tide bundle — never a fabricated
// window list.
// Renamed from an earlier draft's rankTimeWindows to make the scope
// unmistakable in the name itself, not just a comment someone could miss:
// this ranks FORECASTABLE tide/light windows — tide flow (deterministic
// NOAA prediction) + solar/moon timing — never a full future fish-score
// forecast. Wave, wind, water temperature, clarity, and bait have no
// verified forecast source in this app and are never faked here; when
// one exists, this can be expanded to call the full Presence/Feeding/
// Access model per window instead, but that is future work, not now.
async function rankForecastableTideLightWindows({ beachId, speciesId = null, start, end, maxWindows = 3 }) {
  const beach = BEACHES.find((b) => b.id === beachId);
  if (!beach) return { windows: [], error: "Unknown beach." };

  const bundle = await fetchNoaaBundleForBeach(beachId);
  if (!bundle.connected || !bundle.hilo?.ok) {
    return { windows: [], error: bundle.reason || bundle.hilo?.error || "Live tide data unavailable for this beach right now." };
  }

  const moonFactor = scoreMoonFactor(computeMoonPhase(start));
  const horizonMs = end.getTime() - start.getTime();
  const samples = sampleTideWindows(bundle.hilo.data, beach.lat, beach.lon, moonFactor, start, horizonMs, speciesId);
  if (!samples) return { windows: [], error: "Not enough tide data to evaluate this range." };

  // Greedy local-maxima extraction: take the global peak, expand its
  // window the same way predictNextBestBite does, remove those samples
  // from consideration, repeat — same threshold logic, just repeated to
  // surface more than one window.
  let remaining = [...samples];
  const windows = [];
  while (remaining.length > 0 && windows.length < maxWindows) {
    let peakIdx = 0;
    for (let i = 1; i < remaining.length; i++) if (remaining[i].score > remaining[peakIdx].score) peakIdx = i;
    const peak = remaining[peakIdx];
    const threshold = peak.score - 8;
    let startIdx = peakIdx, endIdx = peakIdx;
    while (startIdx > 0 && remaining[startIdx - 1].score >= threshold) startIdx--;
    while (endIdx < remaining.length - 1 && remaining[endIdx + 1].score >= threshold) endIdx++;

    windows.push({
      windowStart: remaining[startIdx].time, windowEnd: remaining[endIdx].time,
      peakTime: peak.time,
      // Deliberately NOT called "score" alone — "tideLightScore" so it can
      // never be confused with a species' full Presence/Feeding/Access
      // final score, which this is not and does not attempt to be.
      tideLightScore: peak.score,
      tideDirection: peak.stage.direction, tideFlowPct: Math.round(peak.stage.flowStrength * 100),
    });
    remaining = remaining.filter((s, i) => i < startIdx || i > endIdx);
  }
  windows.sort((a, b) => b.tideLightScore - a.tideLightScore);
  return {
    windows, beachId, speciesId,
    resultType: "forecastable_tide_light_window", // explicitly NOT "prediction" or "score" — see scope below
    scope: {
      includes: ["tide flow (deterministic NOAA prediction)", "sunrise/sunset low-light timing", "moon phase", ...(speciesId ? ["species tide-direction preference (existing model constant)"] : [])],
      excludes: ["wind", "wave height", "water temperature", "clarity", "bait"],
      reason: "No verified forecast source exists in this app for wind/wave/water-temp/clarity/bait — only tide, sun, and moon are legitimately knowable in advance.",
    },
    description: "This window is ranked from conditions the app can legitimately forecast in advance: tide flow + light/moon. Future wind, waves, water temperature, clarity, and bait are not included. This is NOT the same as a live species score (Presence/Feeding/Access) and should not be interpreted as one.",
  };
}
...
function useLiveSpeciesScores(beachId) {
  ...
  const clarityEstimate = estimateWaterClarity(effectiveBuoy, precipState.precip, tideStage);
  const baitInfo = inferBaitActivity(effectiveBuoy, predictionTimestamp);
  ...
  if (tideStage) {
    for (const [sid, modelFn] of Object.entries(TIDE_MODELS)) {
      const result = modelFn(tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo, isLowLight, predictionTimestamp);
  ...
  const getFishPosition = useCallback((speciesId) => {
    ...
    const pos = predictFishPosition(tideStage, effectiveBuoy, bias, baitInfo);
    return pos ? { ...pos, isEstimate: true } : null;
  }, [tideStage, effectiveBuoy, baitInfo]);
  ...
  return {
    ...
    effectiveBuoy, baitInfo, fishActivity, topSpeciesId, fishability, goStatus, getFishPosition,
            ...
            <div>
              {ranked.slice(0, 5).map((r, i) => (
                <button key={r.beachId} onClick={() => onSelectBeach(r.beachId)} style={{
                  ...
                  <span style={{ color: "#5A7A8A", fontSize: 10.5 }}>{r.confidence}</span>
...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  const {
    ...
    effectiveBuoy, baitInfo, fishActivity, fishability, goStatus, getFishPosition,
    liveResults, refresh,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const clarityAt = estimateWaterClarity(buoyReading, precipReading, tideStageAt);
        ...
        for (const s of SPECIES) {
          ...
          positions[s.id] = predictFishPosition(tideStageAt, buoyReading, bias, baitAt);
        }
          ...
          {goStatus && (() => {
            ...
            return (
              <div style={{ marginBottom: 18 }}>
                ...
                {nextBestBite ? (
                  <div style={{ color: "#8AA6B8", fontSize: 12.5, marginTop: 2 }}>
                    Best window: {nextBestBite.windowStart.toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric", minute: "2-digit" })}
                    {"–"}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                  {topSpeciesEntry && (
                    <span style={{ color: "#5A7A8A", fontSize: 11.5 }}>Confidence: <span style={{ color: "#8AA6B8" }}>{topSpeciesEntry[1].confidence}</span></span>
                  )}
                  {helping.length > 0 && (
            ...
            return (
              <div style={{ marginBottom: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ color: "#7590A0", fontSize: 11, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.5 }}>FISHING PLAN</span>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 14px", marginBottom: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: "#DCE8EE", fontSize: 14, fontWeight: 600 }}>
                      {nextBestBite ? `${nextBestBite.windowStart.toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric", minute: "2-digit" })}–${nextBestBite.windowEnd.toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric", minute: "2-digit" })}` : "Now"}
                    </div>
                  </div>
                  <div>
                    <div style={{ color: "#4A6270", fontSize: 9.5, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4, marginBottom: 2 }}>WHERE</div>
