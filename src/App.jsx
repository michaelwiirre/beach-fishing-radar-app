...
function scoreSpeciesFromProfile(tideStage, moonFactor, buoyFactor, profile, clarityEstimate, isNight) {
  if (!tideStage) return null;
  const factors = [];
  ...
  const score = Math.max(5, Math.min(95, profile.base + flowBonus + directionBonus + moonBonus + waveBonus + pressureBonus + clarityBonus + nightBonus));
  return { score, factors, tideStage };
}
...
const SPECIES_PROFILES = {
  blacktip: { base: 48, flowWeight: 35, bidirectional: true, moonWeight: 1.0, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 1.1, clarityWeight: 0, nightWeight: 1.0 },
  spinner: { base: 46, flowWeight: 32, bidirectional: true, moonWeight: 1.0, wavePreference: "tolerant", waveWeight: 0.9, pressureWeight: 0.9, clarityWeight: 0, nightWeight: 1.0 },
  bull: { base: 50, flowWeight: 38, bidirectional: true, moonWeight: 0.9, wavePreference: "rough", waveWeight: 1.1, pressureWeight: 1.0, clarityWeight: 0, nightWeight: 0.9 },
  tarpon: { base: 42, flowWeight: 22, bidirectional: true, moonWeight: 1.3, wavePreference: "calm", waveWeight: 1.0, pressureWeight: 0.7, clarityWeight: 0.8, nightWeight: 0 },
  snook: { base: 40, flowWeight: 28, bidirectional: false, favoredDirection: "incoming", directionBonus: 6, directionLabel: "Incoming tide favored — ambushes bait at structure on the push", moonWeight: 1.0, wavePreference: "calm", waveWeight: 1.1, pressureWeight: 0.9, clarityWeight: 0.9, nightWeight: 0 },
  jack: { base: 55, flowWeight: 28, bidirectional: true, moonWeight: 0.5, wavePreference: "tolerant", waveWeight: 0.6, pressureWeight: 0.6, clarityWeight: 0.5, nightWeight: 0 },
  bluefish: { base: 48, flowWeight: 30, bidirectional: true, moonWeight: 0.6, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 1.3, clarityWeight: 0.3, nightWeight: 0 },
  whiting: { base: 50, flowWeight: 26, bidirectional: true, moonWeight: 0.5, wavePreference: "calm", waveWeight: 0.9, pressureWeight: 0.6, clarityWeight: 0.7, nightWeight: 0 },
  mackerel: { base: 45, flowWeight: 33, bidirectional: true, moonWeight: 0.8, wavePreference: "calm", waveWeight: 1.2, pressureWeight: 0.9, clarityWeight: 1.0, nightWeight: 0 },
};
...
const TIDE_MODELS = {
  ...
  blacktip: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.blacktip, c, n),
  spinner: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.spinner, c, n),
  bull: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.bull, c, n),
  tarpon: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.tarpon, c, n),
  snook: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.snook, c, n),
  jack: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.jack, c, n),
  bluefish: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.bluefish, c, n),
  whiting: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.whiting, c, n),
  mackerel: (t, m, b, c, n) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.mackerel, c, n),
};
...
function useLiveSpeciesScores(beachId) {
  ...
  if (tideStage) {
    for (const [sid, modelFn] of Object.entries(TIDE_MODELS)) {
      const result = modelFn(tideStage, moonFactor, buoyFactor, clarityEstimate, isNight);
