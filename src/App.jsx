...
// ===========================================================================
// PRIORITY 14 — BEST BEACH COMPARATOR. Loops the EXISTING predictAsOfTimestamp
...
// comparator output is structurally guaranteed identical to what the beach
...
// existing engine.
// ===========================================================================
async function compareBeachesForSpeciesAtTime(speciesId, timestamp) {
  const results = await Promise.all(BEACHES.map(async (beach) => {
    let envelope;
    try { envelope = await predictAsOfTimestamp(beach, speciesId, timestamp); }
    catch { return { beach, available: false }; }
    if (!envelope.result) return { beach, available: false }; // e.g. no tide data at this moment — never fabricate a score, matches predictAsOfTimestamp's own "no prediction possible" contract
    return { beach, available: true, envelope };
  }));
  const ranked = results.filter((r) => r.available)
    .sort((a, b) => b.envelope.result.scoreRaw - a.envelope.result.scoreRaw); // scoreRaw, not rounded score — same precision rankFutureSpeciesWindows already uses to avoid manufacturing ties
...
function buildComparativeExplanation(winner, runnerUp) {
  const w = winner.envelope.result, r = runnerUp.envelope.result;
  ...
  const closeCall = scoreDiff <= 3;
  let text = closeCall
    ? `${winner.beach.name} narrowly leads ${runnerUp.beach.name} (${w.score} vs ${r.score}). Conditions are very similar between the two beaches.`
    : `${winner.beach.name} leads ${runnerUp.beach.name} by ${scoreDiff} points (${w.score} vs ${r.score}).`;

  if (Math.abs(topDriver.diff) >= 2) {
    text += ` The gap comes mainly from ${topDriver.label.toLowerCase()} (${w[topDriver.key]} vs ${r[topDriver.key]}).`;
  }
  ...
  if (winnerTopFactor && winnerTopFactor.delta > 0) text += ` ${winner.beach.name}'s strongest factor: ${winnerTopFactor.label}.`;
  if (runnerUpLimitingFactor && runnerUpLimitingFactor.delta < 0) text += ` ${runnerUp.beach.name}'s main limiting factor: ${runnerUpLimitingFactor.label}.`;
  return text;
}
...
function BestBeachComparatorScreen({ onSelectBeach }) {
  ...
  const [state, setState] = useState({ loading: true, ranked: [], unavailable: [] });
  ...
  useEffect(() => {
    ...
    compareBeachesForSpeciesAtTime(speciesId, selectedTime)
      .then((res) => { if (!cancelled) setState({ loading: false, ranked: res.ranked, unavailable: res.unavailable }); })
      .catch(() => { if (!cancelled) setState({ loading: false, ranked: [], unavailable: [] }); });
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [speciesId, selectedHourIdx]);

  const winner = state.ranked[0];
  const runnerUp = state.ranked[1];
      ...
      {state.loading ? (
        ...
        <>
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ color: "#4A6270", fontSize: 10.5, marginBottom: 8 }}>{hourOptions[selectedHourIdx].label}</div>
            <div onClick={() => onSelectBeach(winner.beach.id)} style={{ cursor: "pointer" }}>
              <div style={{ color: "#E7EFF3", fontSize: 16, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>
                #1 {winner.beach.name} — {winner.envelope.result.score}
              </div>
              <div style={{ color: "#8AA6B8", fontSize: 11, marginTop: 4 }}>
                Presence: {winner.envelope.result.presence} · Feeding: {winner.envelope.result.feeding} · Access: {winner.envelope.result.access} · {winner.envelope.result.confidence} confidence
              </div>
            </div>
            {runnerUp && (
              <div style={{ color: "#17D9C4", fontSize: 11, marginTop: 8 }}>
                +{Math.round(winner.envelope.result.scoreRaw - runnerUp.envelope.result.scoreRaw)} vs {runnerUp.beach.name}
              </div>
            )}
          </div>
          ...
          {explanation && (
            <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: 12, marginBottom: 14 }}>
              <div style={{ color: "#7590A0", fontSize: 10, marginBottom: 4, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4 }}>
                WHY {winner.beach.name.toUpperCase()} RANKS #1
              </div>
              <div style={{ color: "#DCE8EE", fontSize: 12, lineHeight: 1.5 }}>{explanation}</div>
            </div>
          )}

          <div style={{ color: "#7590A0", fontSize: 10, marginBottom: 6, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4 }}>RANKED BEACHES</div>
          {state.ranked.slice(0, 8).map((r, i) => (
            <div key={r.beach.id} onClick={() => onSelectBeach(r.beach.id)} style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "8px 10px", borderRadius: 8, marginBottom: 4, cursor: "pointer",
              background: i === 0 ? "#0F2B28" : "#0E1B26", border: "1px solid #1F3444",
            }}>
              <div>
                <span style={{ color: "#5A7A8A", fontSize: 11, marginRight: 6 }}>{i + 1}.</span>
                <span style={{ color: "#DCE8EE", fontSize: 12.5 }}>{r.beach.name}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ color: "#7590A0", fontSize: 10.5 }}>P{r.envelope.result.presence} F{r.envelope.result.feeding} A{r.envelope.result.access}</span>
                <span style={{ color: i === 0 ? "#17D9C4" : "#DCE8EE", fontSize: 14, fontWeight: 700 }}>{r.envelope.result.score}</span>
              </div>
            </div>
          ))}
          ...
          {state.unavailable.length > 0 && (
            <div style={{ color: "#4A6270", fontSize: 10, marginTop: 10 }}>
              {state.unavailable.length} beach{state.unavailable.length > 1 ? "es" : ""} excluded — insufficient data at this time.
            </div>
          )}
        </>
      )}
