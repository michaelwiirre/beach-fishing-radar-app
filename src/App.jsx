...
// Pompano rule: moving water matters more than direction, but incoming tide
// gets a documented edge (bait fleas washing up the beach face). No moon
// term — solunar theory has little real evidence for a bottom-feeding
// crustacean eater like this. Pressure barely matters either (that's a
// game-fish/blitz-feeder pattern, not a sand-flea forager), so its weight
// here is small.
function scorePompanoFromTide(tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo) {
  ...
  let waveBonus = 0, pressureBonus = 0;
  if (buoyFactor?.buoy) {
    const wb = waveBonusForProfile(buoyFactor.buoy.waveHeightFt, "calm");
    if (wb != null) { waveBonus = wb; factors.push({ label: `Offshore wave height ${buoyFactor.buoy.waveHeightFt.toFixed(1)} ft (calm-water species)`, shortLabel: "Wave", delta: waveBonus }); }
    const pb = pressureBonusBase(buoyFactor.buoy.pressureTendencyHpa);
    if (pb != null) { pressureBonus = Math.round(pb * 0.15); factors.push({ label: `Barometric pressure ${buoyFactor.buoy.pressureTendencyHpa >= 0 ? "+" : ""}${buoyFactor.buoy.pressureTendencyHpa.toFixed(1)} hPa — minor factor for this species`, shortLabel: "Pressure", delta: pressureBonus }); }
  }
  ...
  const bait = baitBonusForProfile(baitInfo, 0.15);
  const baitBonus = bait ? bait.delta : 0;
  if (bait) factors.push(bait);

  const ceiling = computeConditionsCeiling(buoyFactor?.buoy, clarityEstimate);
  const score = Math.max(5, Math.min(ceiling, 50 + flowBonus + incomingBonus + waveBonus + pressureBonus + clarityBonus + baitBonus));
...
// jack crevalle feed aggressively in almost anything, whiting are the
// reliable "always biting" surf staple, and bluefish blitz aggressively in
// rough water ahead of pressure drops. Each profile below weights the SAME
// real inputs (tide, moon, buoy) differently per species — these weights are
...
function scoreSpeciesFromProfile(tideStage, moonFactor, buoyFactor, profile, clarityEstimate, isNight, baitInfo) {
  if (!tideStage) return null;
  const factors = [];
  ...
  let moonBonus = 0;
  if (moonFactor && profile.moonWeight) {
  ...
  const night = nightBonusForProfile(isNight, profile.nightWeight);
  const nightBonus = night ? night.delta : 0;
  if (night) factors.push(night);

  const bait = baitBonusForProfile(baitInfo, profile.baitWeight);
  const baitBonus = bait ? bait.delta : 0;
  if (bait) factors.push(bait);

  const ceiling = computeConditionsCeiling(buoyFactor?.buoy, clarityEstimate);
  const score = Math.max(5, Math.min(ceiling, profile.base + flowBonus + directionBonus + moonBonus + waveBonus + pressureBonus + clarityBonus + nightBonus + baitBonus));
  return { score, factors, tideStage };
}
...
// Only meaningfully affects predator species (baitWeight > 0 in their
// profile) — pompano/whiting are crustacean feeders and barely react. Cut
// from a ±15pt swing to ±6 — this is a weak seasonal prior, not an
// observation, and shouldn't move a score as much as real live tide/wave
// data does. Unknown tier contributes nothing (score is null).
function baitBonusForProfile(baitInfo, weight) {
  if (!baitInfo || !weight || baitInfo.score == null) return null;
  const delta = Math.round(((baitInfo.score - 50) / 50) * 6 * weight);
  return { delta, label: `${baitInfo.level} inferred-seasonal bait likelihood (${baitInfo.basis})`, shortLabel: "Bait" };
}
...
// Fishability = how pleasant/easy the water is to fish, independent of how
// many fish are around. Big surf and heavy wind can mean lots of fish
// activity but a miserable, hard-to-cast session — this is deliberately a
// separate number from the species scores, per the "activity vs
// fishability" distinction.
// Real fishing conditions don't stack linearly — 7ft blown-out surf doesn't
// just subtract points from an otherwise-great tide+moon combo, it caps the
// whole session's ceiling. This runs AFTER the additive factors and clamps
// the final score, instead of being one more term added into the pile.
function computeConditionsCeiling(buoy, clarityEstimate) {
  let ceiling = 95;
  if (buoy?.waveHeightFt != null) {
    if (buoy.waveHeightFt > 6) ceiling = Math.min(ceiling, 55);
    else if (buoy.waveHeightFt > 4.5) ceiling = Math.min(ceiling, 75);
  }
  if (clarityEstimate && clarityEstimate.score < 30) ceiling = Math.min(ceiling, 60);
  return ceiling;
}
...
// weight instead.
// nightWeight: how much this species picks up after dark. Only the three
// sharks get a real value here — the classic "sharks feed more at night"
// pattern; every other species is left at 0 (no factor shown), since that
// wasn't asked for and isn't as well-documented for them in this app's model.
// moonWeight: kept ONLY for tarpon, and small (max ~4 pts). Solunar theory
// is folklore-grade for most surf species; tarpon is the one species with
// genuinely recurring moon-phase-timing claims in FL fishing communities,
// so it gets a small acknowledgment, not the broad ±13pt effect this used
// to apply to 8 of 9 species.
// pressureWeight: kept meaningfully ONLY for bluefish (documented pre-
// frontal blitz behavior) and the 3 sharks (some documented storm-linked
// feeding activity). Bottom/structure feeders (pompano, whiting, snook,
// tarpon, jack, mackerel) have no real literature tying barometric
// pressure to bite quality, so their weight is 0 — no factor shown.
const SPECIES_PROFILES = {
  blacktip: { base: 48, flowWeight: 35, bidirectional: true, moonWeight: 0, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 0.5, clarityWeight: 0, nightWeight: 1.0, zoneBias: 0.4, baitWeight: 0.8 },
  spinner: { base: 46, flowWeight: 32, bidirectional: true, moonWeight: 0, wavePreference: "tolerant", waveWeight: 0.9, pressureWeight: 0.4, clarityWeight: 0, nightWeight: 1.0, zoneBias: 0.3, baitWeight: 0.8 },
  bull: { base: 50, flowWeight: 38, bidirectional: true, moonWeight: 0, wavePreference: "rough", waveWeight: 1.1, pressureWeight: 0.5, clarityWeight: 0, nightWeight: 0.9, zoneBias: 0.6, baitWeight: 0.8 },
  tarpon: { base: 42, flowWeight: 22, bidirectional: true, moonWeight: 0.4, wavePreference: "calm", waveWeight: 1.0, pressureWeight: 0, clarityWeight: 0.8, nightWeight: 0, zoneBias: 0.6, baitWeight: 1.0 },
  snook: { base: 40, flowWeight: 28, bidirectional: false, favoredDirection: "incoming", directionBonus: 6, directionLabel: "Incoming tide favored — ambushes bait at structure on the push", moonWeight: 0, wavePreference: "calm", waveWeight: 1.1, pressureWeight: 0, clarityWeight: 0.9, nightWeight: 0, zoneBias: -0.6, baitWeight: 1.0 },
  jack: { base: 55, flowWeight: 28, bidirectional: true, moonWeight: 0, wavePreference: "tolerant", waveWeight: 0.6, pressureWeight: 0, clarityWeight: 0.5, nightWeight: 0, zoneBias: 0, baitWeight: 0.9 },
  bluefish: { base: 48, flowWeight: 30, bidirectional: true, moonWeight: 0, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 0.6, clarityWeight: 0.3, nightWeight: 0, zoneBias: 0.2, baitWeight: 1.0 },
  whiting: { base: 50, flowWeight: 26, bidirectional: true, moonWeight: 0, wavePreference: "calm", waveWeight: 0.9, pressureWeight: 0, clarityWeight: 0.7, nightWeight: 0, zoneBias: -0.5, baitWeight: 0.1 },
  mackerel: { base: 45, flowWeight: 33, bidirectional: true, moonWeight: 0, wavePreference: "calm", waveWeight: 1.2, pressureWeight: 0, clarityWeight: 1.0, nightWeight: 0, zoneBias: 0.1, baitWeight: 0.6 },
};
...
// Only produces a factor for species with a real nightWeight (the sharks) —
// daytime, or a species this doesn't apply to, shows nothing rather than a
// clutter "+0" line.
function nightBonusForProfile(isNight, weight) {
  if (!isNight || !weight) return null;
  const delta = Math.round(15 * weight);
  return { delta, label: "After dark — this species feeds more actively at night", shortLabel: "Night" };
}
...
const POMPANO_ZONE_BIAS = -0.4; // classic tight trough hunter, same as whiting

const TIDE_MODELS = {
  pompano: scorePompanoFromTide,
  blacktip: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.blacktip, c, n, k),
  spinner: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.spinner, c, n, k),
  bull: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.bull, c, n, k),
  tarpon: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.tarpon, c, n, k),
  snook: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.snook, c, n, k),
  jack: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.jack, c, n, k),
  bluefish: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.bluefish, c, n, k),
  whiting: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.whiting, c, n, k),
  mackerel: (t, m, b, c, n, k) => scoreSpeciesFromProfile(t, m, b, SPECIES_PROFILES.mackerel, c, n, k),
};
...
function useLiveSpeciesScores(beachId) {
  ...
  const getFishPosition = useCallback((speciesId) => {
    const bias = speciesId === "pompano" ? POMPANO_ZONE_BIAS : (SPECIES_PROFILES[speciesId]?.zoneBias || 0);
        ...
        <div style={{ padding: "8px 14px" }}>
          ...
          <div style={{ color: "#4A6270", fontSize: 10.5, marginBottom: 16, lineHeight: 1.4 }}>
            {liveEntries.length > 0
              ? "Live scores are computed from tide, moon, buoy, and (for sight-feeding species) water clarity. Bait presence still isn't wired in. Honest partial scores, not full predictions."
