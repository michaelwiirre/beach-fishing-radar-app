...
// General predator rule (blacktip, spinner, bull, tarpon, snook, jack, cobia,
// mackerel): these push up shallow to feed on the incoming push, then work
// the outgoing as bait gets pulled off the bar \u2014 productive on both legs,
// unlike pompano's incoming bias. Slack water is the one clearly weak stage.
// Water clarity and bait presence still aren't wired in (no live source for
// either) \u2014 wave size and barometric pressure now are, via NDBC.
function scorePredatorFromTide(tideStage, moonFactor, buoyFactor) {
  if (!tideStage) return null;
  const flowBonus = Math.round(tideStage.flowStrength * 35); // 0-35, no directional bias
  const moonBonus = moonFactor ? moonFactor.bonus : 0;
  const buoyBonus = buoyFactor ? buoyFactor.bonus : 0;
  const score = Math.max(5, Math.min(95, 45 + flowBonus + moonBonus + buoyBonus));
  const factors = [
    { label: `${tideStage.direction === "incoming" ? "Incoming" : "Outgoing"} tide, ${Math.round(tideStage.flowStrength * 100)}% of peak flow strength \u2014 both directions count for this species`, delta: flowBonus },
  ];
  if (moonFactor) factors.push({ label: `${moonFactor.moonPhase.name}, ${Math.round(moonFactor.intensity * 100)}% solunar intensity`, delta: moonBonus });
  if (buoyFactor) factors.push(...buoyFactor.factors);
  return { score, factors, tideStage };
...
const TIDE_MODELS = {
  ...
  blacktip: scorePredatorFromTide,
  spinner: scorePredatorFromTide,
  bull: scorePredatorFromTide,
  tarpon: scorePredatorFromTide,
  snook: scorePredatorFromTide,
  jack: scorePredatorFromTide,
  cobia: scorePredatorFromTide,
  mackerel: scorePredatorFromTide,
};
...
function BeachDetailScreen({ beach, onBack, tab, setTab, mode }) {
  ...
  const hasBuoy = isLive && !!NDBC_STATIONS[beach.id];

  const [noaaState, setNoaaState] = useState({ loading: true, bundle: null, fatalError: null });
  const [buoyState, setBuoyState] = useState({ loading: true, buoy: null, fatalError: null });
  ...
  const loadBuoy = useCallback(() => {
    ...
    fetchLatestBuoy(NDBC_STATIONS[beach.id].stationId)
  ...
  // Where live tide + moon + buoy data feeds back into species scores \u2014
  // one model per species. Moon is pure calculation (always available);
  // tide and buoy each depend on their own fetch succeeding independently
  // \u2014 losing one doesn't block the other from still contributing.
