...
// gets a documented edge (bait fleas washing up the beach face). Only rule
// implemented so far \u2014 the only species that can show a LIVE score.
function scorePompanoFromTide(tideStage) {
  ...
  const score = Math.max(5, Math.min(95, 50 + flowBonus + incomingBonus));
  const factors = [
      ...
      {sorted.map((b) => (
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: "#E7EFF3", fontSize: 15, fontWeight: 600 }}>{b.name}</div>
            <div style={{ color: "#5A7A8A", fontSize: 11.5, marginBottom: 6 }}>{b.zone}</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {Object.entries(b.species).sort((x, y) => y[1] - x[1]).slice(0, 3).map(([sid, sc]) => {
...
function BeachDetailScreen({ beach, onBack, tab, setTab, mode }) {
  const isLive = mode === "live" && !!NOAA_STATIONS[beach.id];

  const [noaaState, setNoaaState] = useState({ loading: true, bundle: null, fatalError: null });

  const loadNoaa = useCallback(() => {
    if (!isLive) return;
    setNoaaState({ loading: true, bundle: null, fatalError: null });
    fetchNoaaBundleForBeach(beach.id)
  ...
  // The only place live tide data feeds back into a species score right now.
  const tideStage = (isLive && noaaState.bundle?.hilo?.ok)
    ? computeTideStage(noaaState.bundle.hilo.data, new Date())
    : null;
  const livePompano = tideStage ? scorePompanoFromTide(tideStage) : null;

  const displaySpecies = { ...beach.species };
  const liveSpeciesIds = [];
  if (livePompano) {
    displaySpecies.pompano = livePompano.score;
    liveSpeciesIds.push("pompano");
  }
      ...
      {tab === "overview" ? (
        <div style={{ padding: "8px 14px" }}>
          ...
          <div style={{ color: "#7590A0", fontSize: 11, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.5, marginBottom: 8 }}>
            SPECIES PROBABILITY {livePompano && <span style={{ color: "#17D9C4" }}>(pompano is live)</span>}
          </div>
          ...
          {livePompano && (
            <div style={{ color: "#4A6270", fontSize: 10.5, marginTop: -10, marginBottom: 16, lineHeight: 1.4 }}>
              Pompano is computed from live tide dynamics only \u2014 wave, wind, and water temp aren't wired in yet, so this score reflects tide alone. Every other species here is still demo.
            </div>
          )}
          ...
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: "2px 12px", marginBottom: 16 }}>
            <ConditionRow icon={Wind} label="Wind" value={beach.conditions.wind} />
            <ConditionRow icon={Waves} label="Swell" value={beach.conditions.swell} />
            <ConditionRow icon={Thermometer} label="Water temp" value={beach.conditions.waterTemp} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0" }}>
              <Moon size={14} color="#5A7A8A" />
              <span style={{ color: "#7590A0", fontSize: 12.5, flex: 1 }}>Moon</span>
              <span style={{ color: "#DCE8EE", fontSize: 12.5, fontWeight: 500 }}>{beach.conditions.moon}</span>
            </div>
          </div>
        ...
        <div style={{ padding: "8px 14px" }}>
          {livePompano && (
            <div style={{ background: "#0E1B26", border: "1px solid #17D9C4", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <span style={{ color: "#17D9C4", fontSize: 10.5, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4 }}>
                  LIVE MODEL \u00b7 POMPANO (TIDE-ONLY)
                </span>
                <span style={{ color: "#4A6270", fontSize: 10.5 }}>just now</span>
              </div>
              <div style={{ color: "#DCE8EE", fontSize: 13, marginBottom: 6 }}>
                Computed score: {livePompano.score}/100
              </div>
              {livePompano.factors.map((f, i) => (
              ...
              <div style={{ color: "#4A6270", fontSize: 10.5, marginTop: 6, lineHeight: 1.4 }}>
                Base score 50, adjusted only by tide flow and direction. Wave, wind, water temp, and clarity are not factored in \u2014 not connected yet.
              </div>
            </div>
          )}
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start", background: "#1A1408", border: "1px solid #6B3B1E", borderRadius: 10, padding: 10, marginBottom: 14 }}>
            ...
            <div style={{ color: "#E0C6A8", fontSize: 11.5, lineHeight: 1.4 }}>
              {livePompano
                ? "Everything below is still fabricated demo evidence \u2014 only the pompano card above is a real, computed score."
