...
function useLiveSpeciesScores(beachId) {
  ...
  const getFishPosition = useCallback((speciesId) => {
    ...
    const pos = predictFishPosition(tideStage, effectiveBuoy, bias);
    return pos ? { ...pos, isEstimate: true } : null;
  }, [tideStage, effectiveBuoy]);
...
// Big game vs small game reflects real surf-fishing style, not just raw
// score — a beach can be excellent for one and mediocre for the other.
// Mackerel groups with small game here (light-tackle/schooling species
// culture) despite its "gamefish" tag elsewhere in the app.
const BIG_GAME_SPECIES = ["tarpon", "snook", "blacktip", "spinner", "bull", "jack"];
const SMALL_GAME_SPECIES = ["bluefish", "whiting", "pompano", "mackerel"];
...
function GameStyleScore({ label, score }) {
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ color: "#4A6270", fontSize: 8.5, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.5, marginBottom: 2 }}>{label}</div>
      <div style={{
        color: score != null ? scoreColor(score) : "#5A7A8A", fontSize: 19, fontWeight: 700,
...
function BeachRadarCard({ beach, onSelectBeach }) {
  ...
  const liveEntries = Object.entries(liveResults);
  const bigGameScore = averageForGroup(liveEntries, BIG_GAME_SPECIES);
  const smallGameScore = averageForGroup(liveEntries, SMALL_GAME_SPECIES);
  const hasAnyScore = bigGameScore != null || smallGameScore != null;
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div style={{ display: "flex", gap: 10, flexShrink: 0, width: 76 }}>
          <GameStyleScore label="BIG" score={bigGameScore} />
          <GameStyleScore label="SMALL" score={smallGameScore} />
        </div>
      ) : (
        <div style={{
  ...
  return (
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      <div style={{ color: "#4A6270", fontSize: 11, marginBottom: 14 }}>
        Listed north to south. BIG = tarpon/sharks/snook/jack average, SMALL = bluefish/whiting/pompano/mackerel average, computed live per beach.
      </div>
      {BEACHES.map((b) => (
