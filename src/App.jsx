...
import {
  ...
  classifyWindRelativeToShore, predictAsOfTimestamp, resolveWindForecastSpeed,
...
// ===========================================================================
// LBSF — Cedar Key Shark Predictor. Ported from a standalone Scriptable
// widget (LBSF.js) into this app as its own tab. Deliberately a SEPARATE,
// self-contained module, not folded into scoreSpecies/predictAsOfTimestamp:
// this is a hand-tuned model built from 3 years / 30 logged catches for
// ONE fixed location and ONE fixed bite window, not the generic multi-
// beach/multi-species architecture the rest of this app uses — unifying
// them would misrepresent both. The scoring math below is verbatim from
// the original script; only the data-fetching layer changed, routed
// through this app's own Worker (matching lbsf-worker-routes.js) instead
// of Scriptable's Request/loadJSON, and MULLET_RUN_CONFIRMED became a
// user-toggleable switch instead of a hardcoded constant, since the
// original script's own comment says there's "no way for the script to
// detect this on its own" — a toggle is the natural fit once this has a
// real UI instead of just a widget.
// ===========================================================================
const LBSF_WATER_TEMP_CUTOFF_F = 70;
const LBSF_WINDOW_START_HOUR = 20.5; // 8:30pm
const LBSF_WINDOW_END_HOUR = 24.5;   // 12:30am (next day)

function lbsfWindToNum(windText) {
  const txtVal = String(windText || "");
  const matchVal = txtVal.match(/(\d+)/);
  return matchVal ? Number(matchVal[1]) : 0;
}

function lbsfWeatherPenalty(txtVal) {
  const lowerVal = String(txtVal || "").toLowerCase();
  if (lowerVal.indexOf("thunder") >= 0 || lowerVal.indexOf("storm") >= 0) return -20;
  if (lowerVal.indexOf("showers") >= 0 || lowerVal.indexOf("rain") >= 0) return -8;
  return 0;
}

function lbsfIsTransientWindow(dateVal) {
  const m = dateVal.getMonth() + 1;
  const d = dateVal.getDate();
  return (m === 5 && d >= 21) || (m === 6 && d <= 6);
}

function lbsfIsCoreWindow(tsVal) {
  const h = tsVal.getHours() + tsVal.getMinutes() / 60;
  return h >= LBSF_WINDOW_START_HOUR || h <= (LBSF_WINDOW_END_HOUR - 24);
}

// Score a single night/window. Verbatim port of LBSF.js's scoreRow, with
// mulletRunConfirmed as a parameter (was a hardcoded const in the
// Scriptable version) instead of a module-level toggle.
function lbsfScoreRow(rowVal, waterTempF, mulletRunConfirmed) {
  const reasons = [];
  let score = 0;
  let species = "Bull / Lemon";
  let distance = "150-200 yds (channel edge)";
  let presenceLikely = true;

  if (waterTempF != null) {
    if (waterTempF >= LBSF_WATER_TEMP_CUTOFF_F) {
      score += 45;
      reasons.push("Water " + waterTempF.toFixed(1) + "F, above " + LBSF_WATER_TEMP_CUTOFF_F + "F cutoff");
    } else {
      presenceLikely = false;
      score += 5;
      reasons.push("Water " + waterTempF.toFixed(1) + "F, below cutoff — bulls/lemons likely thin");
    }
  } else {
    reasons.push("No live water temp reading available");
  }

  if (lbsfIsCoreWindow(rowVal.ts)) {
    score += 25;
    reasons.push("In 8:30pm-12:30am core bite window");
  } else {
    reasons.push("Outside core window");
  }

  if (rowVal.tide_direction) {
    score += 20;
    reasons.push("Tide " + rowVal.tide_direction + " — moving water");
  }

  const windPenalty = rowVal.wind_mph >= 20 ? -10 : 0;
  if (windPenalty) reasons.push("Wind " + rowVal.wind_mph + "mph — rough for the kayak run");
  score += windPenalty;

  const wx = lbsfWeatherPenalty(rowVal.forecast_text);
  if (wx) reasons.push(rowVal.forecast_text);
  score += wx;

  if (!presenceLikely && mulletRunConfirmed) {
    score = Math.max(score, 55);
    species = "Sandbar (mullet run)";
    distance = "10-60 yds (tight to shore)";
    reasons.push("Mullet run confirmed (manual override) — sandbars tolerate the cooler water");
  } else if (!presenceLikely) {
    species = "Bull / Lemon unlikely";
  }

  if (lbsfIsTransientWindow(rowVal.ts)) {
    reasons.push("Late May-early June: tiger/hammerhead/Goliath possible too");
  }

  if (rowVal.tide_direction && rowVal.tide_direction !== "unknown" && presenceLikely) {
    distance = "Push to 90-130 yds on strong tide, 150-200 baseline";
  }

  score = Math.max(0, Math.min(100, Math.round(score)));
  return { score, species, distance, reasons };
}

// Data access — routed through this Worker's own /api/lbsf/* routes
// (lbsf-worker-routes.js merged into worker.js), same cachedFetch pattern
// every other data source in this app already uses. Never calls
// NOAA/NWS directly from the browser.
async function fetchLbsfConditions({ force = false } = {}) {
  return cachedFetch("lbsf:conditions", async () => fetchViaProxy("/api/lbsf/conditions", null, {}), { force });
}
async function fetchLbsfForecast({ force = false } = {}) {
  return cachedFetch("lbsf:forecast", async () => fetchViaProxy("/api/lbsf/forecast", null, {}), { force });
}
async function fetchLbsfTrips({ force = false } = {}) {
  return cachedFetch("lbsf:trips", async () => fetchViaProxy("/api/lbsf/trips", null, {}), { force });
}
async function postLbsfTrip(trip) {
  return postToProxy("/api/lbsf/trips", trip);
}

function lbsfFmtNight(dateStr) {
  const d = new Date(dateStr + "T12:00:00");
  return d.toLocaleDateString([], { weekday: "short", month: "short", day: "numeric" });
}
function lbsfFmtTime(ts) {
  return ts.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
}

// Ported from LBSF.js — computed there but never actually used in
// scoring (moon_phase was stored per-row and never referenced by
// scoreRow). Wired in now as a small, explicitly exploratory factor: the
// original model's own analysis of 30 logged catches found no clean
// correlation with moon phase, so this stays a light nudge rather than a
// real driver, and every "why" line says so honestly.
function lbsfMoonPhaseName(dateVal) {
  const lpVal = 2551443;
  const newMoonVal = new Date(Date.UTC(1970, 0, 7, 20, 35, 0));
  const phaseVal = ((dateVal.getTime() - newMoonVal.getTime()) / 1000) % lpVal;
  const fracVal = phaseVal / lpVal;
  if (fracVal < 0.03 || fracVal > 0.97) return "New Moon";
  if (fracVal < 0.22) return "Waxing Crescent";
  if (fracVal < 0.28) return "First Quarter";
  if (fracVal < 0.47) return "Waxing Gibbous";
  if (fracVal < 0.53) return "Full Moon";
  if (fracVal < 0.72) return "Waning Gibbous";
  if (fracVal < 0.78) return "Last Quarter";
  return "Waning Crescent";
}
function lbsfMoonBonus(phaseName) {
  if (phaseName === "New Moon" || phaseName === "Full Moon") return 4; // stronger tidal flow -- the standard hypothesis, unproven in this data
  if (phaseName === "First Quarter" || phaseName === "Last Quarter") return -2;
  return 0;
}

// Finds the closest hourly tide/weather row to any given timestamp —
// ported from LBSF.js's nearestTide, generalized for both row types.
// 90-minute tolerance so a request outside the actually-fetched range
// never silently "matches" something hours away.
function lbsfNearestRow(tsVal, rows, tKey = "t") {
  let best = null, bestDiff = Infinity;
  for (const row of rows) {
    const diff = Math.abs(new Date(row[tKey]).getTime() - tsVal.getTime());
    if (diff < bestDiff) { bestDiff = diff; best = row; }
  }
  return bestDiff <= 90 * 60000 ? best : null;
}

// Real "best bite time" per night — samples candidate half-hours across
// a wide 6pm-3am span (not just the fixed 8:30pm-12:30am check the old
// per-night summary did) and scores each with real tide direction +
// wind + weather + moon at THAT specific hour, picking the best. The
// core-window bonus inside lbsfScoreRow still rewards 8:30pm-12:30am
// specifically — the window your own 20+ logged bites actually happened
// in — so this surfaces a different hour only when the data genuinely
// supports it, not by discarding that history.
function computeLbsfBestBite(dateStr, tideRows, hourlyWeather, waterTempF, mulletRun) {
  const anchor = new Date(dateStr + "T18:00:00"); // 6pm, browser's own local time zone
  const candidates = [];
  for (let stepMin = 0; stepMin <= 9 * 60; stepMin += 30) {
    const ts = new Date(anchor.getTime() + stepMin * 60000);
    const tideRow = lbsfNearestRow(ts, tideRows || []);
    const weatherRow = lbsfNearestRow(ts, hourlyWeather || []);
    const row = {
      ts,
      wind_mph: weatherRow ? weatherRow.windMph : 0,
      tide_direction: tideRow ? tideRow.direction : null,
      forecast_text: weatherRow ? weatherRow.shortForecast : "",
    };
    const scored = lbsfScoreRow(row, waterTempF, mulletRun);
    const moonName = lbsfMoonPhaseName(ts);
    const moonAdj = lbsfMoonBonus(moonName);
    const finalScore = Math.max(0, Math.min(100, scored.score + moonAdj));
    const reasons = [...scored.reasons];
    if (moonAdj !== 0) reasons.push(`${moonName} — ${moonAdj > 0 ? "+" : ""}${moonAdj} (exploratory — weak in your own logged data)`);
    candidates.push({ ts, score: finalScore, species: scored.species, distance: scored.distance, reasons, moonName, tideDirection: tideRow?.direction ?? null });
  }
...
function LbsfScreen() {
  const [conditions, setConditions] = useState({ loading: true, data: null, error: null });
  const [forecast, setForecast] = useState({ loading: true, data: null, error: null });
  const [tripsState, setTripsState] = useState({ loading: true, data: null, error: null });
  const [mulletRun, setMulletRun] = useState(false);
  const [logOpen, setLogOpen] = useState(false);
  const [logForm, setLogForm] = useState({ date: new Date().toISOString().slice(0, 10), species: "", sighted: false, caught: false, count: "", distanceYd: "", notes: "" });
  const [logSaving, setLogSaving] = useState(false);
  const [logResult, setLogResult] = useState(null);
  const [expandedNight, setExpandedNight] = useState(null);

  const loadAll = useCallback((force = false) => {
    setConditions((s) => ({ ...s, loading: true }));
    fetchLbsfConditions({ force }).then((data) => setConditions({ loading: false, data, error: null }))
      .catch((err) => setConditions({ loading: false, data: null, error: err.message }));
    setForecast((s) => ({ ...s, loading: true }));
    fetchLbsfForecast({ force }).then((data) => setForecast({ loading: false, data, error: null }))
      .catch((err) => setForecast({ loading: false, data: null, error: err.message }));
    setTripsState((s) => ({ ...s, loading: true }));
    fetchLbsfTrips({ force }).then((data) => setTripsState({ loading: false, data, error: null }))
      .catch((err) => setTripsState({ loading: false, data: null, error: err.message }));
  }, []);
  ...
  const nowScore = (() => {
    ...
    const row = {
      ts: now,
      wind_mph: conditions.data.windSpeed != null ? Math.round(conditions.data.windSpeed) : 0,
      tide_direction: conditions.data.tideDirection,
      forecast_text: "",
    };
    const scored = lbsfScoreRow(row, conditions.data.waterTempF, mulletRun);
    const moonName = lbsfMoonPhaseName(now);
    const moonAdj = lbsfMoonBonus(moonName);
    const finalScore = Math.max(0, Math.min(100, scored.score + moonAdj));
    const reasons = [...scored.reasons];
    if (moonAdj !== 0) reasons.push(`${moonName} — ${moonAdj > 0 ? "+" : ""}${moonAdj} (exploratory — weak in your own logged data)`);
    return { ...scored, score: finalScore, reasons };
  })();

  const nightsScored = (() => {
    if (!forecast.data) return [];
    const { waterTempF, tideRows, hourlyWeather } = forecast.data;
    const out = [];
    for (let i = 0; i < 7; i++) {
      const d = new Date(Date.now() + i * 86400000);
      const dateStr = d.toISOString().slice(0, 10);
      const best = computeLbsfBestBite(dateStr, tideRows, hourlyWeather, waterTempF, mulletRun);
      if (best) out.push({ date: dateStr, isEstimate: i > 0, ...best });
    }
    return out;
  })();

  async function submitLbsfTrip() {
    setLogSaving(true);
    setLogResult(null);
    try {
      await postLbsfTrip({ ...logForm, loggedAt: new Date().toISOString() });
      setLogResult({ ok: true });
      setLogForm({ date: new Date().toISOString().slice(0, 10), species: "", sighted: false, caught: false, count: "", distanceYd: "", notes: "" });
      fetchLbsfTrips({ force: true }).then((data) => setTripsState({ loading: false, data, error: null }));
    } catch (err) {
      setLogResult({ ok: false, error: err.message });
    } finally {
      setLogSaving(false);
  ...
  return (
    <div style={{ padding: "14px 14px 100px" }}>
      <div style={{ color: "#7590A0", fontSize: 11, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.5, marginBottom: 2 }}>
        CEDAR KEY SHARK PREDICTOR
      </div>
      <div style={{ color: "#4A6270", fontSize: 10.5, marginBottom: 14 }}>
        Built from 3 years / 30 logged catches · fixed 8:30pm–12:30am window
      </div>

      {conditions.loading ? (
        <div style={{ color: "#5A7A8A", fontSize: 12, marginBottom: 14 }}>Loading conditions…</div>
      ) : conditions.error ? (
        <div style={{ color: "#FF5D5D", fontSize: 12, marginBottom: 14 }}>Couldn't load conditions: {conditions.error}</div>
      ) : nowScore && (
        <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div style={{ color: "#E7EFF3", fontSize: 22, fontWeight: 800, fontFamily: "'Space Grotesk', sans-serif" }}>{nowScore.score}</div>
            <div style={{ color: "#8AA6B8", fontSize: 12 }}>{nowScore.species}</div>
          </div>
          <div style={{ color: "#7590A0", fontSize: 11, marginTop: 4 }}>{nowScore.distance}</div>
          <div style={{ color: "#4A6270", fontSize: 10.5, marginTop: 8 }}>
            Water {conditions.data.waterTempF != null ? `${conditions.data.waterTempF.toFixed(1)}°F` : "n/a"}
            {" · "}Tide {conditions.data.tideDirection || "n/a"}
            {" · "}Pressure {conditions.data.pressureTrend || "n/a"}
            {" · "}Wind {conditions.data.windSpeed != null ? `${Math.round(conditions.data.windSpeed)} kt` : "n/a"}
          </div>
          {nowScore.reasons.map((r, i) => (
            <div key={i} style={{ color: "#8AA6B8", fontSize: 10.5, marginTop: 3 }}>{r}</div>
          ))}
        </div>
      )}

      <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, cursor: "pointer" }}>
        <input type="checkbox" checked={mulletRun} onChange={(e) => setMulletRun(e.target.checked)} />
        <span style={{ color: "#DCE8EE", fontSize: 12.5 }}>Mullet run confirmed (eyes on it right now)</span>
      </label>

      <div style={{ color: "#7590A0", fontSize: 10, marginBottom: 6, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4 }}>
        NEXT 7 NIGHTS — BEST BITE TIME
      </div>
      {forecast.loading ? (
        <div style={{ color: "#5A7A8A", fontSize: 12 }}>Loading forecast…</div>
      ) : forecast.error ? (
        <div style={{ color: "#FF5D5D", fontSize: 12 }}>Couldn't load forecast: {forecast.error}</div>
      ) : (
        nightsScored.map((n) => {
          const isExpanded = expandedNight === n.date;
          return (
            <div key={n.date} style={{ borderRadius: 8, background: "#0E1B26", border: "1px solid #1F3444", marginBottom: 4, overflow: "hidden" }}>
              <div onClick={() => setExpandedNight(isExpanded ? null : n.date)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", cursor: "pointer" }}>
                <div>
                  <div style={{ color: "#DCE8EE", fontSize: 12.5 }}>{lbsfFmtNight(n.date)} · {lbsfFmtTime(n.ts)}</div>
                  <div style={{ color: "#5A7A8A", fontSize: 10.5 }}>{n.species} · tide {n.tideDirection || "n/a"}{n.isEstimate ? " · water temp estimated" : ""}</div>
                </div>
                <span style={{ color: n.score >= 55 ? "#17D9C4" : "#8AA6B8", fontSize: 16, fontWeight: 700 }}>{n.score}</span>
              </div>
              {isExpanded && (
                <div style={{ padding: "0 10px 10px" }}>
                  <div style={{ color: "#7590A0", fontSize: 10.5, marginBottom: 4 }}>{n.distance}</div>
                  {n.reasons.map((r, i) => (
                    <div key={i} style={{ color: "#8AA6B8", fontSize: 10.5, marginTop: 2 }}>{r}</div>
                  ))}
                </div>
              )}
            </div>
      ...
      <button onClick={() => setLogOpen(!logOpen)} style={{
        width: "100%", marginTop: 16, padding: "10px 0", background: "none",
        border: "1px dashed #2A3F4E", borderRadius: 8, color: "#F5A623", fontSize: 12.5, cursor: "pointer",
      }}>
        {logOpen ? "Close" : "Log This Trip"}
      </button>

      {logOpen && (
        <div style={{ marginTop: 10 }}>
          <input type="date" value={logForm.date} onChange={(e) => setLogForm({ ...logForm, date: e.target.value })}
            style={{ width: "100%", background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 6, padding: 8, color: "#DCE8EE", marginBottom: 8 }} />
          <input placeholder="Species (Bull, Lemon, Sandbar...)" value={logForm.species} onChange={(e) => setLogForm({ ...logForm, species: e.target.value })}
            style={{ width: "100%", background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 6, padding: 8, color: "#DCE8EE", marginBottom: 8 }} />
          <div style={{ display: "flex", gap: 14, marginBottom: 8 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#DCE8EE", fontSize: 12.5 }}>
              <input type="checkbox" checked={logForm.sighted} onChange={(e) => setLogForm({ ...logForm, sighted: e.target.checked })} /> Sighted
            </label>
            <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#DCE8EE", fontSize: 12.5 }}>
              <input type="checkbox" checked={logForm.caught} onChange={(e) => setLogForm({ ...logForm, caught: e.target.checked })} /> Caught
            </label>
          </div>
          <input placeholder="Count" value={logForm.count} onChange={(e) => setLogForm({ ...logForm, count: e.target.value })}
            style={{ width: "100%", background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 6, padding: 8, color: "#DCE8EE", marginBottom: 8 }} />
          <input placeholder="Distance (yd)" value={logForm.distanceYd} onChange={(e) => setLogForm({ ...logForm, distanceYd: e.target.value })}
            style={{ width: "100%", background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 6, padding: 8, color: "#DCE8EE", marginBottom: 8 }} />
          <textarea placeholder="Notes" value={logForm.notes} onChange={(e) => setLogForm({ ...logForm, notes: e.target.value })}
            style={{ width: "100%", background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 6, padding: 8, color: "#DCE8EE", marginBottom: 8, minHeight: 60 }} />
          <button onClick={submitLbsfTrip} disabled={logSaving} style={{
            width: "100%", padding: "10px 0", background: "#17D9C4", border: "none", borderRadius: 8,
            color: "#04211D", fontWeight: 700, fontSize: 13, cursor: logSaving ? "default" : "pointer", opacity: logSaving ? 0.6 : 1,
          }}>
            {logSaving ? "Saving…" : "Save Trip"}
          </button>
          {logResult?.ok === true && <div style={{ color: "#17D9C4", fontSize: 12, marginTop: 8 }}>Saved.</div>}
          {logResult?.ok === false && <div style={{ color: "#FF5D5D", fontSize: 12, marginTop: 8 }}>Couldn't save: {logResult.error}</div>}
        </div>
      )}

      <div style={{ color: "#7590A0", fontSize: 10, marginTop: 20, marginBottom: 6, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4 }}>
        LOGGED TRIPS
      </div>
      {tripsState.loading ? (
        <div style={{ color: "#5A7A8A", fontSize: 12 }}>Loading…</div>
      ) : tripsState.error ? (
        <div style={{ color: "#FF5D5D", fontSize: 12 }}>Couldn't load trips: {tripsState.error}</div>
      ) : !tripsState.data || tripsState.data.length === 0 ? (
        <div style={{ color: "#5A7A8A", fontSize: 12 }}>No trips logged yet.</div>
      ) : (
        [...tripsState.data].sort((a, b) => new Date(b.date) - new Date(a.date)).map((t, i) => (
          <div key={i} style={{ padding: "8px 10px", borderRadius: 8, background: "#0E1B26", border: "1px solid #1F3444", marginBottom: 4 }}>
            <div style={{ color: "#DCE8EE", fontSize: 12.5 }}>{t.date} — {t.species || "unspecified"}</div>
            <div style={{ color: "#5A7A8A", fontSize: 10.5 }}>
              {t.sighted ? "Sighted" : ""}{t.sighted && t.caught ? " · " : ""}{t.caught ? "Caught" : ""}
              {t.count ? ` · ${t.count}` : ""}{t.distanceYd ? ` · ${t.distanceYd}yd` : ""}
            </div>
            {t.notes && <div style={{ color: "#4A6270", fontSize: 10.5, marginTop: 2 }}>{t.notes}</div>}
          </div>
        ))
      )}
    </div>
...
export default function BeachFishingRadar() {
  ...
  return (
    <div style={{
      ...
      {screen === "lbsf" && <ErrorBoundary key="lbsf"><LbsfScreen /></ErrorBoundary>}
