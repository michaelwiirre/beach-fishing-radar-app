...
function accessDistanceScore(yd) {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    if (yd >= x0 && yd <= x1) return Math.round(y0 + (y1 - y0) * (yd - x0) / (x1 - x0));
  }
}
...
function computeAccess(speciesId, position, windKt, flowStrength) {
  ...
  const [minYd, maxYd] = position.distanceYd;
  const midYd = (minYd + maxYd) / 2 / gearScaleFor(speciesId);
  const distScore = accessDistanceScore(midYd);
  const windPenalty = accessWindPenalty(windKt);
  const currentPenalty = accessCurrentPenalty((flowStrength || 0) * 100);
  const access = Math.max(0, Math.min(100, distScore - windPenalty - currentPenalty));
...
function scoreSpecies(species, tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo, isLowLight) {
...
const TIDE_MODELS = {
  pompano: (t, m, b, c, n, k, l) => scoreSpecies("pompano", t, m, b, c, n, k, l),
  blacktip: (t, m, b, c, n, k, l) => scoreSpecies("blacktip", t, m, b, c, n, k, l),
  spinner: (t, m, b, c, n, k, l) => scoreSpecies("spinner", t, m, b, c, n, k, l),
  bull: (t, m, b, c, n, k, l) => scoreSpecies("bull", t, m, b, c, n, k, l),
  tarpon: (t, m, b, c, n, k, l) => scoreSpecies("tarpon", t, m, b, c, n, k, l),
  snook: (t, m, b, c, n, k, l) => scoreSpecies("snook", t, m, b, c, n, k, l),
  jack: (t, m, b, c, n, k, l) => scoreSpecies("jack", t, m, b, c, n, k, l),
  bluefish: (t, m, b, c, n, k, l) => scoreSpecies("bluefish", t, m, b, c, n, k, l),
  whiting: (t, m, b, c, n, k, l) => scoreSpecies("whiting", t, m, b, c, n, k, l),
  mackerel: (t, m, b, c, n, k, l) => scoreSpecies("mackerel", t, m, b, c, n, k, l),
};
...
function useLiveSpeciesScores(beachId) {
  ...
  const baitInfo = inferBaitActivity(effectiveBuoy, new Date());

  const beach = BEACHES.find((b) => b.id === beachId);
  const isNight = beach ? computeIsNight(new Date(), beach.lat, beach.lon) : false;
  const isLowLight = beach ? computeIsLowLight(new Date(), beach.lat, beach.lon) : false;

  const liveResults = {}; // speciesId -> { score, factors, tideStage }
  if (tideStage) {
    for (const [sid, modelFn] of Object.entries(TIDE_MODELS)) {
      const result = modelFn(tideStage, moonFactor, buoyFactor, clarityEstimate, isNight, baitInfo, isLowLight);
...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const buoyRows = ndbcStation ? await fetchBuoyHistoryDay(ndbcStation, dateCompactStr) : [];
        const buoyReading = pickNearestRow(buoyRows, targetDate);
        const precipRows = metarStation ? await fetchWeatherHistoryDay(metarStation, dateCompactStr) : [];
        const precipNearest = pickNearestRow(precipRows, targetDate);
        ...
        if (tideStageAt) {
          for (const [sid, modelFn] of Object.entries(TIDE_MODELS)) {
            const result = modelFn(tideStageAt, moonFactorAt, buoyFactorAt, clarityAt, isNightAt, baitAt, isLowLightAt);
        ...
        if (!cancelled) {
          setLogRecon({
            ...
            tideDirection: tideStageAt?.direction ?? null, tideFlowPct: tideStageAt ? Math.round(tideStageAt.flowStrength * 100) : null,
            baitTier: baitAt?.tier ?? null, baitLevel: baitAt?.level ?? null,
            moonPhaseName: moonFactorAt?.moonPhase?.name ?? null, moonIllumPct: moonFactorAt ? Math.round(moonFactorAt.moonPhase.illumination * 100) : null,
            scored, positions,
          });
        }
      } catch (err) {
  ...
  const submitTripLog = async () => {
    ...
    const payload = {
      ...
      wave_ft: logRecon.waveFt, wave_ft_source: logRecon.waveSource,
      wind_kt: logRecon.windKt, wind_kt_source: logRecon.windSource,
      ...
      tide_direction: logRecon.tideDirection, tide_flow_pct: logRecon.tideFlowPct,
      bait_tier: logRecon.baitTier, bait_level: logRecon.baitLevel,
      moon_phase_name: logRecon.moonPhaseName, moon_illumination_pct: logRecon.moonIllumPct,
      observations,
    };
    try {
      ...
      {tab === "overview" ? (
        <div style={{ padding: "8px 14px" }}>
          ...
          <button
            ...
            style={{
              width: "100%", padding: "12px 0", borderRadius: 10, border: "1px solid #F5A623",
              background: logOpen ? "#2A2008" : "#0E1B26", color: "#F5A623", fontSize: 13,
              fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4, cursor: "pointer", marginBottom: logOpen ? 8 : 14,
            }}
          >
          ...
          {logOpen && (
            <div style={{ background: "#0E1B26", border: "1px solid #F5A623", borderRadius: 10, padding: 14, marginBottom: 14 }}>
              <div style={{ color: "#7590A0", fontSize: 11, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.5, marginBottom: 4 }}>
                LOG THIS TRIP
              </div>
          ...
          <div style={{ background: "#0E1B26", border: "1px solid #1F3444", borderRadius: 10, padding: "2px 12px", marginBottom: 4 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: "1px solid #16232E" }}>
              <Waves size={14} color={effectiveBuoy?.waveHeightFt != null ? (effectiveBuoy.waveHeightIsEstimated ? "#F5A623" : effectiveBuoy.waveHeightIsRegional ? "#3E8FFF" : "#17D9C4") : "#5A7A8A"} />
              <span style={{ color: "#7590A0", fontSize: 12.5, flex: 1 }}>
                Swell (offshore) {effectiveBuoy?.waveHeightFt != null && (
                  effectiveBuoy.waveHeightIsEstimated
                    ? <span style={{ color: "#F5A623", fontSize: 9 }}>● ESTIMATED</span>
                    : effectiveBuoy.waveHeightIsRegional
                      ? <span style={{ color: "#3E8FFF", fontSize: 9 }}>● REGIONAL</span>
                      : <span style={{ color: "#17D9C4", fontSize: 9 }}>● LIVE</span>
                )}
              </span>
              <span style={{ color: "#DCE8EE", fontSize: 12.5, fontWeight: 500 }}>
                {effectiveBuoy?.waveHeightFt != null
                  ? `${effectiveBuoy.waveHeightFt.toFixed(1)} ft${effectiveBuoy.dominantWavePeriodS != null && !effectiveBuoy.waveHeightIsEstimated ? ` @ ${effectiveBuoy.dominantWavePeriodS}s` : ""}`
                  : "Not available"}
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: "1px solid #16232E" }}>
