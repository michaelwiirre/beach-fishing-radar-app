...
function scoreSpeciesFromProfile(tideStage, moonFactor, buoyFactor, profile, clarityEstimate, isNight, baitInfo) {
  ...
  const score = Math.max(5, Math.min(95, profile.base + flowBonus + directionBonus + moonBonus + waveBonus + pressureBonus + clarityBonus + nightBonus + baitBonus));
  return { score, factors, tideStage };
}
...
// ===========================================================================
// BAIT ACTIVITY — explicitly INFERRED, never a real observation. There is no
// live bait sensor anywhere in this app. This is a coarse behavioral guess
// from water temperature (baitfish are more active in a comfortable range)
// and the well-known Florida fall mullet run window. Every place this shows
// up in the UI is labeled INFERRED, matching the app's honesty rules.
// ===========================================================================
function inferBaitActivity(buoy, now) {
  if (!buoy || buoy.waterTempF == null) return null;
  const t = buoy.waterTempF;
  let level, score;
  if (t >= 72 && t <= 84) { level = "High"; score = 78; }
  else if ((t >= 66 && t < 72) || (t > 84 && t <= 88)) { level = "Moderate"; score = 55; }
  else { level = "Low"; score = 32; }
  const month = now.getUTCMonth() + 1;
  let seasonNote = null;
  if (month >= 9 && month <= 11) { score = Math.min(95, score + 12); seasonNote = "fall mullet run window"; }
  return {
    level, score,
    basis: `Inferred from water temp ${Math.round(t)}°F${seasonNote ? ` + ${seasonNote}` : ""} — no live bait sensor exists`,
  };
}

// Only meaningfully affects predator species (baitWeight > 0 in their
// profile) — pompano/whiting are crustacean feeders and barely react.
function baitBonusForProfile(baitInfo, weight) {
  if (!baitInfo || !weight) return null;
  const delta = Math.round(((baitInfo.score - 50) / 50) * 15 * weight);
  return { delta, label: `${baitInfo.level} inferred bait activity (${baitInfo.basis})`, shortLabel: "Bait" };
}
...
const SPECIES_PROFILES = {
  blacktip: { base: 48, flowWeight: 35, bidirectional: true, moonWeight: 1.0, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 1.1, clarityWeight: 0, nightWeight: 1.0, zoneBias: 0.4, baitWeight: 0.8 },
  spinner: { base: 46, flowWeight: 32, bidirectional: true, moonWeight: 1.0, wavePreference: "tolerant", waveWeight: 0.9, pressureWeight: 0.9, clarityWeight: 0, nightWeight: 1.0, zoneBias: 0.3, baitWeight: 0.8 },
  bull: { base: 50, flowWeight: 38, bidirectional: true, moonWeight: 0.9, wavePreference: "rough", waveWeight: 1.1, pressureWeight: 1.0, clarityWeight: 0, nightWeight: 0.9, zoneBias: 0.6, baitWeight: 0.8 },
  tarpon: { base: 42, flowWeight: 22, bidirectional: true, moonWeight: 1.3, wavePreference: "calm", waveWeight: 1.0, pressureWeight: 0.7, clarityWeight: 0.8, nightWeight: 0, zoneBias: 0.6, baitWeight: 1.0 },
  snook: { base: 40, flowWeight: 28, bidirectional: false, favoredDirection: "incoming", directionBonus: 6, directionLabel: "Incoming tide favored — ambushes bait at structure on the push", moonWeight: 1.0, wavePreference: "calm", waveWeight: 1.1, pressureWeight: 0.9, clarityWeight: 0.9, nightWeight: 0, zoneBias: -0.6, baitWeight: 1.0 },
  jack: { base: 55, flowWeight: 28, bidirectional: true, moonWeight: 0.5, wavePreference: "tolerant", waveWeight: 0.6, pressureWeight: 0.6, clarityWeight: 0.5, nightWeight: 0, zoneBias: 0, baitWeight: 0.9 },
  bluefish: { base: 48, flowWeight: 30, bidirectional: true, moonWeight: 0.6, wavePreference: "rough", waveWeight: 1.0, pressureWeight: 1.3, clarityWeight: 0.3, nightWeight: 0, zoneBias: 0.2, baitWeight: 1.0 },
  whiting: { base: 50, flowWeight: 26, bidirectional: true, moonWeight: 0.5, wavePreference: "calm", waveWeight: 0.9, pressureWeight: 0.6, clarityWeight: 0.7, nightWeight: 0, zoneBias: -0.5, baitWeight: 0.1 },
  mackerel: { base: 45, flowWeight: 33, bidirectional: true, moonWeight: 0.8, wavePreference: "calm", waveWeight: 1.2, pressureWeight: 0.9, clarityWeight: 1.0, nightWeight: 0, zoneBias: 0.1, baitWeight: 0.6 },
};
...
function useLiveSpeciesScores(beachId) {
  ...
  const fishActivity = liveEntries.length > 0
  ...
  const getFishPosition = useCallback((speciesId) => {
    ...
    return predictFishPosition(tideStage, buoyState.buoy, bias);
            ...
            <div style={{ background: "#0E1B26", border: `1.5px solid ${goStatus.color}`, borderRadius: 12, padding: 16, marginBottom: 12, textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1, background: "#070D14", borderRadius: 8, padding: "8px 6px" }}>
                  <div style={{ color: "#4A6270", fontSize: 9.5, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4, marginBottom: 2 }}>FISH ACTIVITY</div>
              ...
              <div style={{ color: "#5A7A8A", fontSize: 10.5, marginTop: 8, lineHeight: 1.4 }}>
                Fish Activity = how likely species are biting. Fishability = how easy/pleasant conditions are to fish. They can disagree — e.g. lots of fish around but rough surf makes it a tough session.
              </div>
            </div>
          )}
                ...
                <>
                  <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                    <div style={{ flex: 1, background: "#070D14", borderRadius: 8, padding: "8px 10px" }}>
                      <div style={{ color: "#4A6270", fontSize: 9.5, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4, marginBottom: 3 }}>FISH POSITION</div>
                  ...
                  <div style={{ color: "#4A6270", fontSize: 10, marginTop: 4, lineHeight: 1.4 }}>
                    Inferred from surf height + tide movement — no real bathymetry data source exists for this beach, so treat this as an educated read, not measured structure.
                  </div>
                </>
              ) : (
            ...
            <div style={{ background: "#0E1B26", border: "1px solid #17D9C4", borderRadius: 10, padding: 14, marginBottom: 14 }}>
              ...
              {baitInfo && (
                <div style={{ marginBottom: 10 }}>
                  ...
                  <span style={{ color: "#DCE8EE", fontSize: 12.5 }}>{baitInfo.level} (inferred)</span>
                </div>
              )}
