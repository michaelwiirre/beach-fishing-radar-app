...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  const [regionalForageState, setRegionalForageState] = useState({ loading: true, data: null });
  useEffect(() => {
    let cancelled = false;
    setRegionalForageState({ loading: true, data: null });
    fetch(`${PROXY_BASE_URL}/forage/evidence?beachId=${encodeURIComponent(beach.id)}`)
      .then((r) => r.json())
      .then((data) => { if (!cancelled) setRegionalForageState({ loading: false, data }); })
      .catch(() => { if (!cancelled) setRegionalForageState({ loading: false, data: null }); });
    return () => { cancelled = true; };
  }, [beach.id]);
            ...
            return (
              <div style={{ marginBottom: 18 }}>
                ...
                {/* ============ REGIONAL FORAGE EVIDENCE (Priority 13, informational
                    ...
                    as such explicitly, not just by omission. ============ */}
                {regionalForageState.data?.supported && (
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px dashed #2A3F4E" }}>
                    ...
                    {regionalForageState.loading ? (
                      <div style={{ color: "#5A7A8A", fontSize: 11 }}>Checking regional reports…</div>
                        ...
                        {regionalForageState.data.evidence.slice(0, regionalForageExpanded ? 10 : 3).map((ev, i) => (
                          <div key={i} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: i < regionalForageState.data.evidence.length - 1 ? "1px solid #16232E" : "none" }}>
                            <div style={{ color: "#DCE8EE", fontSize: 12, fontWeight: 600 }}>
                              {ev.forageType.replace("_", " ")}
                              {ev.concentration && ev.concentration !== "unknown" ? ` — ${ev.concentration}` : ev.presence === "scarce" ? " — scarce" : ""}
                              {ev.movement ? ` · ${ev.movement.replace("_", " ")}` : ""}
                            </div>
