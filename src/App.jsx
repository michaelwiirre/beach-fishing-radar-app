      ...
      {days.map((d) => {
        ...
        return (
          <button key={d.offset} onClick={() => onSelect(d.offset)} style={{
            ...
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 800, color: d.score == null ? "#4A6270" : scoreColor(d.score), marginTop: 5 }}>{d.loading ? "…" : d.score ?? "—"}</div>
            <div style={{ fontSize: 8.5, color: "#5A7A8A", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.bestSpeciesName || "Forecast"}</div>
          </button>
        );
      })}
    </div>
...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [beach.id, selectedForecastDay?.bestSpeciesId, selectedForecastDay?.offset]);
              ...
              <div style={{ marginTop: 11, padding: 11, background: "#09141D", border: "1px solid #162A37", borderRadius: 11 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 9 }}>
                  <div>
                    ...
                    <div style={{ color: "#DCE8EE", fontSize: 14, fontWeight: 700 }}>{selectedForecastDay.weekday} {selectedForecastDay.dateLabel} · {selectedForecastDay.bestSpeciesName || "No species forecast"}</div>
                  ...
                  <div style={{ color: scoreColor(selectedForecastDay.score ?? 0), fontSize: 24, fontWeight: 800, fontFamily: "'Space Grotesk', sans-serif" }}>{selectedForecastDay.score ?? "—"}</div>
                </div>
