...
import {
  ...
  dateToCompact, dateToEasternCompact, pickLatestAvailableRow, scoreBuoyFactor, estimateWaterClarity, normalizeObservations,
  ...
  classifyWindRelativeToShore, predictAsOfTimestamp, resolveWindForecastSpeed, resolveSatelliteClarity,
...
async function fetchTideHistoryDay(stationId, dateCompactStr, { force = false } = {}) {
  return cachedFetch(`tide-history:${stationId}:${dateCompactStr}`, async () => {
    ...
    const rows = normalizeNoaaTideRows(json.predictions, "UTC"); // Priority 15 Stage 2 — shared helper, not an inline duplicate
...
async function runBacktest({ beachId = null, speciesId = null, startDate = null, endDate = null } = {}) {
  ...
  // Every (trip, species) combination that passes the filters below gets
  // flattened into one job list, then run in parallel — this used to be a
  // fully sequential nested loop (one predictAsOfTimestamp at a time, trip
  // outer / species inner), which is what made this take minutes with any
  // real number of logged trips: 20 trips x 10 species was 200 predictions
  // run one after another. Every filter and every field below is otherwise
  // unchanged; only the execution order changed.
  const jobs = [];
  for (const trip of trips) {
    ...
    for (const sid of speciesToTest) {
      jobs.push({ trip, beach, sid, predictionTimestamp, tripHasAnyActivity });
    }
  }

  const settled = await Promise.allSettled(
    jobs.map((job) => predictAsOfTimestamp(job.beach, job.sid, job.predictionTimestamp, browserDataAccess))
  );

  const results = [];
  for (let i = 0; i < jobs.length; i++) {
    const { trip, beach, sid, predictionTimestamp, tripHasAnyActivity } = jobs[i];
    const outcome = settled[i];
    if (outcome.status === "rejected") {
      results.push({
        beachId: beach.id, speciesId: sid, predictionTimestamp: predictionTimestamp.toISOString(),
        error: outcome.reason?.message, verdict: "insufficient_evidence", verdictReason: `Replay failed: ${outcome.reason?.message}`,
      });
      continue;
    }
...
async function rankFutureSpeciesWindows({ beachId, speciesId, start, end, maxWindows = 3, stepMinutes = 30 }) {
  ...
  // Best Bite samples the next 24h at 30-minute resolution (~49 model
  // evaluations). These used to run strictly one-after-another. Even though
  // browserDataAccess de-duplicates the underlying network requests, each
  // model evaluation still had to finish before the next one could begin,
  // which could leave the UI sitting on "Finding best bite window…" for a
  // very long time on mobile. Start all timestamp evaluations together. The
  // existing cachedFetch/inFlight layer still collapses identical upstream
  // requests, so this changes scheduling only — not scoring, inputs, or the
  // resulting window ranking. Promise.allSettled also lets one bad timestamp
  // fail without discarding every other valid sample.
  const timestamps = [];
  for (let t = start.getTime(); t <= end.getTime(); t += stepMs) timestamps.push(new Date(t));

  const settled = await Promise.allSettled(
    timestamps.map((ts) => predictAsOfTimestamp(beach, speciesId, ts, browserDataAccess))
  );
  const samples = [];
  for (let i = 0; i < settled.length; i++) {
    ...
    if (!envelope?.result) continue; // no valid prediction at this moment — skip honestly, never fabricate a score
    samples.push({
      time: timestamps[i],
      slotIndex: i, // preserve the original 30-minute grid so failed samples remain real gaps
      scoreRaw: envelope.result.scoreRaw,
  ...
  // Local peaks — only genuinely adjacent grid samples are neighbors. A
  // rejected/null prediction remains a hard gap in the curve instead of
  // being silently deleted and making the valid samples on either side
  // look 30 minutes apart. This prevents a missing sample from bridging
  // two otherwise separate bite opportunities.
  const peakIndices = [];
  const isAdjacent = (leftIdx, rightIdx) =>
    leftIdx >= 0 && rightIdx < samples.length &&
    samples[rightIdx].slotIndex - samples[leftIdx].slotIndex === 1;
  for (let i = 0; i < samples.length; i++) {
    const hasPrev = isAdjacent(i - 1, i);
    const hasNext = isAdjacent(i, i + 1);
    const prevOk = !hasPrev || samples[i].scoreRaw >= samples[i - 1].scoreRaw;
    const nextOk = !hasNext || samples[i].scoreRaw >= samples[i + 1].scoreRaw;
  ...
  let rawWindows = peakIndices.map((peakIdx) => {
    ...
    let startIdx = peakIdx, endIdx = peakIdx;
    while (startIdx > 0 && isAdjacent(startIdx - 1, startIdx) && samples[startIdx - 1].scoreRaw >= floor) startIdx--;
    while (endIdx < samples.length - 1 && isAdjacent(endIdx, endIdx + 1) && samples[endIdx + 1].scoreRaw >= floor) endIdx++;
    ...
    if (startIdx === endIdx) {
      // Widen only across real adjacent samples; never stretch a display
      // window across a failed prediction slot just to make it look wider.
      if (startIdx > 0 && isAdjacent(startIdx - 1, startIdx)) startIdx--;
      if (endIdx < samples.length - 1 && isAdjacent(endIdx, endIdx + 1)) endIdx++;
    }
  ...
  for (const w of rawWindows) {
    ...
    if (last && w.startIdx <= last.endIdx + 1 &&
        samples[w.startIdx].slotIndex - samples[last.endIdx].slotIndex <= 1) {
  ...
  return {
    ...
    // Every sampled point across the range, not just the clustered peak
    // windows above — added for the hourly bite-time chart, which needs
    // the real per-slot score curve, not a re-derived approximation.
    // Reuses this exact same prediction pass rather than re-fetching.
    samples: samples.map((s) => ({ time: s.time, score: s.score, scoreRaw: s.scoreRaw })),
  };
}
...
// Priority 15 Stage 3 — model-wide forecast accuracy, computed server-side
// by the scheduled cron (issueForecastVerificationBatch/verifyDueForecastSnapshots
// in worker.js): real forecasts get issued, then re-checked once their
// target time has actually passed, using real historical observations —
// not a client-side replay. Genuinely different question from the
// Calibration Check below (which tests score-to-catch correlation from
// YOUR logged trips): this tests whether the FORECASTS THEMSELVES
// (wave/wind/tide, days out) turned out to be right, aggregated across
// every beach/species this Worker tracks, not just what you've logged.
async function fetchForecastVerificationMetrics() {
  return fetchViaProxy("/forecast-verification/metrics", null, {});
}
...
const browserDataAccess = {
  ...
  fetchMarineForecastForBeach, fetchWindForecastForBeach, fetchSatelliteClarityForBeach,
};
...
// Priority 18 — NWS periods forecast (human-readable shortForecast text +
// icon per day), routed through the Worker's own /forecast/periods.
// Mirrors fetchForecastForBeach exactly. Separate from the gridpoint fetch
// above, which only carries raw numeric layers.
async function fetchForecastPeriodsForBeach(beach, { force = false } = {}) {
  if (!beach) return null;
  return cachedFetch(`forecastPeriods:${beach.id}`, async () => {
    return fetchViaProxy("/forecast/periods", null, { lat: beach.lat, lon: beach.lon });
  }, { force });
}
...
// Priority 16 — satellite clarity (NOAA CoastWatch VIIRS Kd490, routed
// through the Worker's own /clarity/satellite). Mirrors
// fetchWindForecastForBeach exactly. A pure supplement to the existing
// wave/wind/precip-based clarity estimate, never a replacement — real
// satellite readings are less reliable in shallow nearshore water and
// have real gaps from cloud cover, so the estimate stays the reliable
// fallback whenever no fresh reading exists.
async function fetchSatelliteClarityForBeach(beach, { force = false } = {}) {
  if (!beach) return null;
  return cachedFetch(`satclarity:${beach.id}`, async () => {
    return fetchViaProxy("/clarity/satellite", null, { lat: beach.lat, lon: beach.lon });
  }, { force });
}
...
function useLiveSpeciesScores(beachId) {
  ...
  const [windFallbackState, setWindFallbackState] = useState({ loading: false, reading: null });
  const [satelliteClarityState, setSatelliteClarityState] = useState({ loading: false, reading: null });
  ...
  // Priority 15.2 — same idea as the wave fallback above, for wind. Once
  // the primary buoy resolves, if it genuinely has no wind reading (down,
  // maintenance, a real anemometer failure — a buoy can report wave/temp
  // fine while its wind sensor specifically is out), fall back to
  // HRRR/GFS via the SAME fetchWindForecastForBeach this app's future-
  // prediction path already uses — reusing that function, not
  // reimplementing the fetch.
  useEffect(() => {
    if (!hasBuoy || buoyState.loading || !buoyState.buoy || buoyState.buoy.windSpeedKt != null) {
      setWindFallbackState({ loading: false, reading: null });
      return;
    }
    let cancelled = false;
    const beachObj = BEACHES.find((b) => b.id === beachId);
    if (!beachObj) { setWindFallbackState({ loading: false, reading: null }); return; }
    setWindFallbackState({ loading: true, reading: null });
    fetchWindForecastForBeach(beachObj)
      .then((bundle) => {
        if (cancelled) return;
        const resolved = bundle ? resolveWindForecastSpeed(bundle, new Date()) : null;
        setWindFallbackState({ loading: false, reading: resolved?.windSpeedKt != null ? resolved : null });
      })
      .catch(() => { if (!cancelled) setWindFallbackState({ loading: false, reading: null }); });
    return () => { cancelled = true; };
  }, [beachId, hasBuoy, buoyState.loading, buoyState.buoy]);

  // Priority 16 — satellite clarity for THIS live-conditions pipeline
  // specifically (the one this screen actually uses — separate from
  // predictAsOfTimestamp, which already gets this via Best Beach). Always
  // attempted once per beach load, not conditional on the estimate
  // failing — this is a supplement/cross-check, not a fallback, so it's
  // fetched regardless of whether clarityEstimate below succeeds.
  useEffect(() => {
    let cancelled = false;
    const beachObj = BEACHES.find((b) => b.id === beachId);
    if (!beachObj) { setSatelliteClarityState({ loading: false, reading: null }); return; }
    setSatelliteClarityState({ loading: true, reading: null });
    fetchSatelliteClarityForBeach(beachObj)
      .then((bundle) => {
        if (cancelled) return;
        const resolved = bundle ? resolveSatelliteClarity(bundle, new Date()) : null;
        setSatelliteClarityState({ loading: false, reading: resolved });
      })
      .catch(() => { if (!cancelled) setSatelliteClarityState({ loading: false, reading: null }); });
    return () => { cancelled = true; };
  }, [beachId]);
  ...
  // only as a last resort, a wind-derived Beaufort-scale estimate.
  // Wind speed fallback (Priority 15.2): the beach's own live buoy
  // reading, else HRRR/GFS via fetchWindForecastForBeach — the same
  // function the future-prediction path already uses, not a
  // reimplementation. Independent of the wave fallback above — a buoy can
  // report wave fine while its wind sensor specifically is down, or vice
  // versa, so each is resolved on its own rather than one gating the
  // other. Each tier is tagged distinctly so the UI never shows a
  // fallback as LIVE.
  const effectiveBuoy = (() => {
    if (!buoyState.buoy) return null;
    let result = buoyState.buoy;
    if (result.waveHeightFt == null) {
      if (regionalWaveState.reading) {
        ...
        if (estimated != null) result = { ...result, waveHeightFt: estimated, waveHeightIsEstimated: true };
      }
    }
    if (result.windSpeedKt == null && windFallbackState.reading) {
      result = {
        ...result,
        windSpeedKt: windFallbackState.reading.windSpeedKt,
        windDirDeg: windFallbackState.reading.windDirDeg,
        windSpeedIsHrrrGfs: true,
      };
    }
    return result;
  })();
...
// ===========================================================================
// FORECAST DASHBOARD — compact, fishing-first presentation layer.
// IMPORTANT: this does not create a second scoring formula. Every daily score
// below is a real predictAsOfTimestamp() result from the shared species model.
// The dashboard only reorganizes those existing predictions into a faster
// planning surface: date strip -> conditions -> species outlook -> trends.
// ===========================================================================
// Priority 18 — simple, dependency-free icon mapping for weather
// condition text (NWS shortForecast) and moon phase, matching the
// lightweight emoji-icon style the rest of this app already uses
// elsewhere rather than pulling in a weather-icon library for this.
...
function moonIconFor(moonPhaseName) {
  const t = (moonPhaseName || "").toLowerCase();
  if (t.includes("new")) return "🌑";
  if (t.includes("waxing crescent")) return "🌒";
  if (t.includes("first quarter")) return "🌓";
  if (t.includes("waxing gibbous")) return "🌔";
  if (t.includes("full")) return "🌕";
  if (t.includes("waning gibbous")) return "🌖";
  if (t.includes("last quarter") || t.includes("third quarter")) return "🌗";
  if (t.includes("waning crescent")) return "🌘";
  return "🌙";
}
...
function ForecastMetric({ label, value, sub, accent = false }) {
  return (
    <div style={{ background: "#0B1720", border: "1px solid #162A37", borderRadius: 10, padding: "10px 11px", minHeight: 58 }}>
      <div style={{ color: "#5A7A8A", fontSize: 9.5, letterSpacing: 0.6, fontFamily: "'Space Grotesk', sans-serif", marginBottom: 4 }}>{label}</div>
      <div style={{ color: accent ? "#17D9C4" : "#E7EFF3", fontSize: 15, fontWeight: 700 }}>{value ?? "—"}</div>
      {sub ? <div style={{ color: "#5A7A8A", fontSize: 9.5, marginTop: 2 }}>{sub}</div> : null}
    </div>
  );
}

function MiniTrend({ title, data, valueKey, suffix = "", decimals = 0 }) {
  const usable = (data || []).filter((d) => d[valueKey] != null);
  if (usable.length < 2) return null;
  return (
    <div style={{ marginTop: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ color: "#B7CBD6", fontSize: 12, fontWeight: 600 }}>{title}</span>
        <span style={{ color: "#5A7A8A", fontSize: 10 }}>7-day trend</span>
      </div>
      <div style={{ height: 92, background: "#09141D", border: "1px solid #162A37", borderRadius: 10, padding: "6px 2px 0" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={usable} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
            <XAxis dataKey="day" tick={{ fill: "#5A7A8A", fontSize: 9 }} axisLine={false} tickLine={false} />
            <YAxis hide domain={["dataMin - 1", "dataMax + 1"]} />
            <Area type="monotone" dataKey={valueKey} stroke="#17D9C4" fill="#17D9C4" fillOpacity={0.08} strokeWidth={2} isAnimationActive={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4, color: "#4A6270", fontSize: 9.5 }}>
        <span>{Number(usable[0][valueKey]).toFixed(decimals)}{suffix}</span>
        <span>{Number(usable[usable.length - 1][valueKey]).toFixed(decimals)}{suffix}</span>
      </div>
    </div>
  );
}

// Hourly bite-activity curve for one specific day — the real per-30-minute
// species score samples rankFutureSpeciesWindows already computed while
// finding the Best Fishing Window (samples field), not a re-derived
// approximation, with that window shaded so the number and the picture
// agree. Mirrors MiniTrend's visual style exactly.
function BiteTimeChart({ samples, windowStart, windowEnd }) {
  const usable = (samples || []).filter((s) => s.score != null);
  if (usable.length < 2) return null;
  const data = usable.map((s) => ({ t: new Date(s.time).getTime(), score: s.score }));
  const winStartMs = windowStart ? new Date(windowStart).getTime() : null;
  const winEndMs = windowEnd ? new Date(windowEnd).getTime() : null;
  return (
    <div style={{ marginBottom: 9 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ color: "#B7CBD6", fontSize: 12, fontWeight: 600 }}>Bite Activity by Hour</span>
        <span style={{ color: "#5A7A8A", fontSize: 10 }}>this day</span>
      </div>
      <div style={{ height: 96, background: "#09141D", border: "1px solid #162A37", borderRadius: 10, padding: "6px 2px 0" }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
            <XAxis dataKey="t" type="number" domain={["dataMin", "dataMax"]} scale="time"
              tickFormatter={(v) => new Date(v).toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric" })}
              tick={{ fill: "#5A7A8A", fontSize: 8.5 }} axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={40} />
            <YAxis hide domain={[0, 100]} />
            {winStartMs != null && winEndMs != null && (
              <ReferenceArea x1={winStartMs} x2={winEndMs} fill="#17D9C4" fillOpacity={0.14} stroke="none" />
            )}
            <Area type="monotone" dataKey="score" stroke="#17D9C4" fill="#17D9C4" fillOpacity={0.1} strokeWidth={2} isAnimationActive={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function SevenDayStrip({ days, selectedOffset, onSelect }) {
  return (
    <div style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
      {days.map((d) => {
        const selected = d.offset === selectedOffset;
        return (
          <button key={d.offset} onClick={() => onSelect(d.offset)} style={{
            minWidth: 72, borderRadius: 11, padding: "8px 7px", cursor: "pointer",
            border: `1px solid ${selected ? "#17D9C4" : "#1B303E"}`,
            background: selected ? "#102A2C" : "#0A151E", color: "#DCE8EE", textAlign: "center",
          }}>
            <div style={{ fontSize: 10, color: selected ? "#8FE8DC" : "#7590A0", fontWeight: 600 }}>{d.weekday}</div>
            <div style={{ fontSize: 9.5, color: "#4A6270", marginTop: 1 }}>{d.dateLabel}</div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 800, color: d.conditionsScore == null ? "#4A6270" : scoreColor(d.conditionsScore), marginTop: 5 }}>{d.loading ? "…" : d.conditionsScore ?? "—"}</div>
            <div style={{ fontSize: 8.5, color: "#5A7A8A", marginTop: 2 }}>Conditions</div>
          </button>
        );
      })}
    </div>
...
function BeachDetailScreen({ beach, onBack, tab, setTab }) {
  ...
  // null = "show whichever species the model picked as best for this
  // day" (existing default behavior). Once the user picks one, every
  // downstream piece (day-at-a-glance card, Best Fishing Window, the
  // hourly chart) switches to that species specifically.
  const [selectedOutlookSpeciesId, setSelectedOutlookSpeciesId] = useState(null);
  const [dailyOutlook, setDailyOutlook] = useState({ loading: true, days: [] });
  const [showForecastTrends, setShowForecastTrends] = useState(false);
  ...
  const {
    ...
    liveResults, refresh, satelliteClarityState,
  ...
  // Seven-day outlook: one representative midday prediction per species/day,
  // then the best species for that day becomes the beach-level planning score.
  // This intentionally reuses the existing shared model instead of inventing
  // a generic weather score. All upstream bundles are cached, so the many
  // model evaluations do not imply one network request per species.
  useEffect(() => {
    let cancelled = false;
    setDailyOutlook({ loading: true, days: [] });
    const now = new Date();
    const jobs = Array.from({ length: 7 }, (_, offset) => (async () => {
      const target = new Date(now);
      target.setDate(target.getDate() + offset);
      target.setHours(12, 0, 0, 0);
      const settled = await Promise.allSettled(SPECIES.map((sp) => predictAsOfTimestamp(beach, sp.id, target, browserDataAccess)));
      const valid = settled.map((x, i) => x.status === "fulfilled" && x.value?.result ? { species: SPECIES[i], envelope: x.value } : null).filter(Boolean);
      valid.sort((a, b) => b.envelope.result.scoreRaw - a.envelope.result.scoreRaw);
      const best = valid[0] || null;
      const env = best?.envelope?.environmentalInputs || {};
      const fp = best?.envelope?.forecastProvenance?.fields || {};
      // Species-independent conditions score for the day-selector cards —
      // reuses computeFishability exactly as the live "now" GO FISH status
      // already does elsewhere (pure wave/wind favorability, no species
      // involved), rather than the auto-picked best species' own score.
      const conditionsScore = computeFishability(env.waveFt != null || env.windKt != null ? { waveHeightFt: env.waveFt, windSpeedKt: env.windKt } : null)?.score ?? null;
      // Every species' own result for this day, keyed by id — lets the
      // species selector show any species' detail instantly without a
      // second prediction pass, since this same call already computed it.
      const bySpecies = Object.fromEntries(valid.map((v) => [v.species.id, v.envelope]));
      return {
        offset,
        date: target,
        weekday: target.toLocaleDateString("en-US", { timeZone: "America/New_York", weekday: "short" }),
        dateLabel: target.toLocaleDateString("en-US", { timeZone: "America/New_York", month: "short", day: "numeric" }),
        score: best?.envelope?.result?.score ?? null,
        conditionsScore,
        bestSpeciesId: best?.species?.id ?? null,
        bestSpeciesName: best?.species?.name ?? null,
        envelope: best?.envelope ?? null,
        bySpecies,
        windKt: env.windKt ?? null, waveFt: env.waveFt ?? null, wavePeriodS: env.wavePeriodS ?? null,
        waterTempF: env.waterTempF ?? null, clarityScore: env.clarityScore ?? null,
        pressureHpa: fp.pressure?.value ?? null,
        moon: env.moonPhaseName ?? null, tideDirection: env.tideDirection ?? null, tideFlowPct: env.tideFlowPct ?? null,
      };
    })());
    // Priority 18 — weather condition text/icon per day. Fetched once
    // (one NWS call returns all upcoming periods), then matched to each
    // of the 7 days by date, picking that day's DAYTIME period since
    // that's what a "Weather: Light Rain Shower" summary should reflect.
    const periodsJob = fetchForecastPeriodsForBeach(beach).catch(() => null);
    Promise.all([Promise.all(jobs), periodsJob]).then(([days, periodsBundle]) => {
      if (cancelled) return;
      const periods = periodsBundle?.periods || [];
      const withWeather = days.map((d) => {
        const dayKey = d.date.toLocaleDateString("en-US", { timeZone: "America/New_York" });
        const match = periods.find((p) => p.isDaytime && new Date(p.startTime).toLocaleDateString("en-US", { timeZone: "America/New_York" }) === dayKey);
        return { ...d, shortForecast: match?.shortForecast || null, weatherIcon: match?.icon || null };
      });
      setDailyOutlook({ loading: false, days: withWeather });
    });
    return () => { cancelled = true; };
  }, [beach.id]);

  const forecastDays = dailyOutlook.days.length ? dailyOutlook.days : Array.from({ length: 7 }, (_, offset) => {
    const d = new Date(); d.setDate(d.getDate() + offset);
    return { offset, weekday: d.toLocaleDateString("en-US", { weekday: "short" }), dateLabel: d.toLocaleDateString("en-US", { month: "short", day: "numeric" }), loading: true };
  });
  const selectedForecastDay = dailyOutlook.days.find((d) => d.offset === selectedDayOffset) || null;
  // The species actually driving the day-at-a-glance detail below: the
  // user's explicit pick if they made one, else whichever species the
  // model found best for this day (unchanged default behavior).
  const activeOutlookSpeciesId = selectedOutlookSpeciesId || selectedForecastDay?.bestSpeciesId || null;
  const activeOutlookSpeciesName = SPECIES.find((sp) => sp.id === activeOutlookSpeciesId)?.name ?? selectedForecastDay?.bestSpeciesName ?? null;

  // Priority 18 — "recommendations on fish activity at any given time
  // and why," for whichever day is selected in the 7-day strip, not just
  // "now". Reuses rankFutureSpeciesWindows exactly as the existing
  // "now" Best Bite feature does (bestBiteState below) — same real
  // model, same honest-unavailable behavior, same whyLabels/peakResult
  // shape — just scoped to the selected day's full 00:00-23:59 window
  // instead of the next 24h from this instant, and now to whichever
  // species is active (user-picked or auto-best) rather than always auto.
  const [dayBestWindowState, setDayBestWindowState] = useState({ loading: true, result: null, error: null });
  useEffect(() => {
    const speciesId = activeOutlookSpeciesId;
    if (!speciesId || !selectedForecastDay?.date) { setDayBestWindowState({ loading: false, result: null, error: null }); return; }
    let cancelled = false;
    setDayBestWindowState({ loading: true, result: null, error: null });
    const dayStart = new Date(selectedForecastDay.date);
    dayStart.setHours(0, 0, 0, 0);
    const dayEnd = new Date(selectedForecastDay.date);
    dayEnd.setHours(23, 59, 0, 0);
    rankFutureSpeciesWindows({ beachId: beach.id, speciesId, start: dayStart, end: dayEnd, maxWindows: 2 })
      .then((res) => {
        if (cancelled) return;
        if (res.error || !res.windows || res.windows.length === 0) {
          setDayBestWindowState({ loading: false, result: null, error: res.error || "No valid prediction available for this day." });
        } else {
          setDayBestWindowState({ loading: false, result: res, error: null });
        }
      })
      .catch((err) => {
        if (cancelled) return;
        setDayBestWindowState({ loading: false, result: null, error: err.message });
      });
    return () => { cancelled = true; };
  }, [beach.id, activeOutlookSpeciesId, selectedForecastDay?.offset]);
  const dayBestWindow = dayBestWindowState.result?.windows?.[0] ?? null;

  // Sunrise/sunset for the selected day — computeSunTimes already exists
  // and is already used internally for low-light scoring; this just
  // surfaces it in the UI for the first time.
  const selectedDaySun = selectedForecastDay?.date ? computeSunTimes(selectedForecastDay.date, beach.lat, beach.lon) : null;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const tideDateCompactStr = dateToEasternCompact(targetDate);
        ...
        const tideRows = noaaStation ? await fetchTideHistoryDay(noaaStation, tideDateCompactStr) : null;
    ...
    <div style={{ paddingBottom: 90 }}>
      ...
      {tab === "overview" ? (
        <div style={{ padding: "10px 14px" }}>
          {/* ============ 7-DAY FISHING DASHBOARD ============ */}
          <div style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
              <div>
                <div style={{ color: "#E7EFF3", fontSize: 15, fontWeight: 700 }}>7-Day Fishing Outlook</div>
                <div style={{ color: "#5A7A8A", fontSize: 10.5 }}>Species-model forecast · tap a day to inspect it</div>
              </div>
              {dailyOutlook.loading ? <span style={{ color: "#5A7A8A", fontSize: 10 }}>building…</span> : null}
            </div>
            <SevenDayStrip days={forecastDays} selectedOffset={selectedDayOffset} onSelect={setSelectedDayOffset} />

            {selectedForecastDay && (
              <div style={{ marginTop: 11, padding: 11, background: "#09141D", border: "1px solid #162A37", borderRadius: 11 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 9 }}>
                  <div>
                    <div style={{ color: "#5A7A8A", fontSize: 9.5, letterSpacing: 0.5 }}>DAY AT A GLANCE</div>
                    <div style={{ color: "#DCE8EE", fontSize: 14, fontWeight: 700 }}>{selectedForecastDay.weekday} {selectedForecastDay.dateLabel} · {activeOutlookSpeciesName || "No species forecast"}</div>
                    {selectedForecastDay.shortForecast && (
                      <div style={{ color: "#7590A0", fontSize: 11, marginTop: 2 }}>{weatherIconFor(selectedForecastDay.shortForecast)} {selectedForecastDay.shortForecast}</div>
                    )}
                  </div>
                  <div style={{ color: scoreColor((selectedOutlookSpeciesId ? selectedForecastDay.bySpecies?.[selectedOutlookSpeciesId]?.result?.score : selectedForecastDay.score) ?? 0), fontSize: 24, fontWeight: 800, fontFamily: "'Space Grotesk', sans-serif" }}>
                    {selectedOutlookSpeciesId ? (selectedForecastDay.bySpecies?.[selectedOutlookSpeciesId]?.result?.score ?? "—") : (selectedForecastDay.score ?? "—")}
                  </div>
                </div>

                {/* Species picker — the day cards above now show a raw,
                    species-independent conditions score; picking a species
                    here drives everything below (Best Fishing Window, the
                    hourly chart) to that species specifically instead of
                    whichever one the model auto-picked as best. */}
                <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 6, marginBottom: 2, scrollbarWidth: "none" }}>
                  <button onClick={() => setSelectedOutlookSpeciesId(null)} style={{
                    whiteSpace: "nowrap", borderRadius: 8, padding: "5px 10px", fontSize: 11, cursor: "pointer",
                    border: `1px solid ${!selectedOutlookSpeciesId ? "#17D9C4" : "#1B303E"}`,
                    background: !selectedOutlookSpeciesId ? "#102A2C" : "transparent", color: !selectedOutlookSpeciesId ? "#8FE8DC" : "#7590A0",
                  }}>Best match</button>
                  {SPECIES.map((sp) => (
                    <button key={sp.id} onClick={() => setSelectedOutlookSpeciesId(sp.id)} style={{
                      whiteSpace: "nowrap", borderRadius: 8, padding: "5px 10px", fontSize: 11, cursor: "pointer",
                      border: `1px solid ${selectedOutlookSpeciesId === sp.id ? "#17D9C4" : "#1B303E"}`,
                      background: selectedOutlookSpeciesId === sp.id ? "#102A2C" : "transparent", color: selectedOutlookSpeciesId === sp.id ? "#8FE8DC" : "#7590A0",
                    }}>{sp.name}</button>
                  ))}
                </div>

                {/* Best Fishing Window + why — the actual species model,
                    sampled across this specific day, not a generic tide
                    proxy. Mirrors the "now" Best Bite feature exactly
                    (rankFutureSpeciesWindows), just scoped to whichever
                    day is selected. */}
                {dayBestWindowState.loading ? (
                  <div style={{ color: "#5A7A8A", fontSize: 11.5, marginBottom: 9 }}>Finding best window for this day…</div>
                ) : dayBestWindow ? (
                  <div style={{ background: "#0E2620", border: "1px solid #1D4A3C", borderRadius: 9, padding: "9px 11px", marginBottom: 9 }}>
                    <div style={{ color: "#5A7A8A", fontSize: 9, letterSpacing: 0.5 }}>BEST FISHING WINDOW</div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginTop: 2 }}>
                      <span style={{ color: "#5FEFC0", fontSize: 15, fontWeight: 700 }}>
                        {fmtClockTime(dayBestWindow.windowStart)} — {fmtClockTime(dayBestWindow.windowEnd)}
                      </span>
                      <span style={{ color: scoreColor(dayBestWindow.score), fontSize: 15, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>{dayBestWindow.score}</span>
                    </div>
                    {dayBestWindow.whyLabels?.length > 0 && (
                      <div style={{ color: "#8AA6B8", fontSize: 10.5, marginTop: 3 }}>Why: {dayBestWindow.whyLabels.join(" · ")}</div>
                    )}
                  </div>
                ) : dayBestWindowState.error ? (
                  <div style={{ color: "#5A7A8A", fontSize: 11, marginBottom: 9 }}>Best window unavailable: {dayBestWindowState.error}</div>
                ) : null}

                <BiteTimeChart samples={dayBestWindowState.result?.samples} windowStart={dayBestWindow?.windowStart} windowEnd={dayBestWindow?.windowEnd} />

                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 7 }}>
                  <ForecastMetric label="SURF" value={selectedForecastDay.waveFt != null ? `${selectedForecastDay.waveFt.toFixed(1)} ft` : "—"} sub={selectedForecastDay.wavePeriodS != null ? `${selectedForecastDay.wavePeriodS.toFixed(1)}s period` : "forecast"} />
                  <ForecastMetric label="WIND" value={selectedForecastDay.windKt != null ? `${Math.round(selectedForecastDay.windKt)} kt` : "—"} sub="speed" />
                  <ForecastMetric label="CLARITY" value={selectedForecastDay.clarityScore != null ? `${selectedForecastDay.clarityScore}/100` : "—"} sub="estimated" accent />
                  <ForecastMetric label="WATER" value={selectedForecastDay.waterTempF != null ? `${selectedForecastDay.waterTempF.toFixed(1)}°F` : "—"} sub={selectedForecastDay.envelope?.environmentalInputs?.waterTempSource === "observed" ? "observed" : "unavailable"} />
                  <ForecastMetric label="TIDE" value={selectedForecastDay.tideDirection ? selectedForecastDay.tideDirection.toUpperCase() : "—"} sub={selectedForecastDay.tideFlowPct != null ? `${selectedForecastDay.tideFlowPct}% flow` : "midday"} />
                  <ForecastMetric label="MOON" value={selectedForecastDay.moon ? `${moonIconFor(selectedForecastDay.moon)} ${selectedForecastDay.moon}` : "—"} sub="phase" />
                </div>

                {selectedDaySun && (
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 9, padding: "8px 2px", borderTop: "1px solid #162A37" }}>
                    <span style={{ color: "#8AA6B8", fontSize: 11 }}>☀️ Sunrise {fmtClockTime(selectedDaySun.sunrise)}</span>
                    <span style={{ color: "#8AA6B8", fontSize: 11 }}>🌙 Sunset {fmtClockTime(selectedDaySun.sunset)}</span>
                  </div>
                )}

                <button onClick={() => setShowForecastTrends(!showForecastTrends)} style={{ width: "100%", marginTop: 9, background: "transparent", border: "1px solid #1F3444", borderRadius: 8, color: "#8AA6B8", padding: "7px 9px", fontSize: 10.5, cursor: "pointer" }}>
                  {showForecastTrends ? "Hide forecast trends" : "View 7-day forecast trends"}
                </button>
                {showForecastTrends && (
                  <div>
                    <MiniTrend title="Surf Height" data={dailyOutlook.days} valueKey="waveFt" suffix=" ft" decimals={1} />
                    <MiniTrend title="Wind" data={dailyOutlook.days} valueKey="windKt" suffix=" kt" decimals={0} />
                    <MiniTrend title="Estimated Clarity" data={dailyOutlook.days} valueKey="clarityScore" suffix="" decimals={0} />
                    <MiniTrend title="Pressure" data={dailyOutlook.days.map((d) => ({ ...d, pressureIn: d.pressureHpa != null ? d.pressureHpa * 0.0295299830714 : null }))} valueKey="pressureIn" suffix=" in" decimals={2} />
                    <MiniTrend title="Water Temperature" data={dailyOutlook.days} valueKey="waterTempF" suffix="°F" decimals={1} />
                  </div>
                )}
              </div>
            )}
          ...
          <div style={{ marginBottom: 18 }}>
              ...
              return (
                <div>
                  ...
                  <Row icon={Wind} label="Wind" keyRow={isKey("Wind")}
                    live={effectiveBuoy?.windSpeedKt != null ? (effectiveBuoy.windSpeedIsHrrrGfs ? "MODEL" : "LIVE") : null}
                    value={effectiveBuoy?.windSpeedKt != null ? `${Math.round(effectiveBuoy.windSpeedKt)} kt` : "Not available"} />
            ...
            {showMoreConditions && (
              <div style={{ marginTop: 8 }}>
                ...
                {/* Priority 16 — real satellite reading, shown ONLY when
                    a fresh cloud-free VIIRS pass actually exists (won't
                    always be true) — a labeled supplement/cross-check
                    to the estimate above, never a silent override. */}
                {satelliteClarityState.reading && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #16232E" }}>
                    <Eye size={13} color="#3E8FFF" />
                    <span style={{ color: "#7590A0", fontSize: 12.5, flex: 1 }}>Clarity <span style={{ color: "#3E8FFF", fontSize: 8.5 }}> ● SATELLITE (VIIRS)</span></span>
                    <span style={{ color: "#DCE8EE", fontSize: 12.5 }}>{satelliteClarityState.reading.label} · {formatSatelliteAge(satelliteClarityState.reading.ageHours)}</span>
                  </div>
                )}
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #16232E" }}>
...
// Builds ONE consistent conditions summary from environmentalInputs — the
// same real values every beach entry already carries, regardless of which
// species is selected or how that species happens to weight each factor.
// Used identically for the winner and every ranked-list row, so wind
// (or any other condition) never silently drops out just because it
// didn't rank in a species-specific top-5-by-delta list.
// Ages can legitimately run up to 10 days for satellite readings (see
// resolveSatelliteClarity's staleness cutoff) — "212h old" reads poorly
// at that scale, so this switches to days once it's large enough to matter.
function formatSatelliteAge(ageHours) {
  if (ageHours < 48) return `${ageHours}h old`;
  return `${Math.round(ageHours / 24 * 10) / 10}d old`;
}

function formatConditionsSummary(ei) {
  if (!ei) return "Conditions unavailable";
  const parts = [
    ei.tideDirection ? `Tide ${ei.tideDirection}${ei.tideFlowPct != null ? ` ${ei.tideFlowPct}%` : ""}` : "Tide n/a",
    ei.waveFt != null ? `Wave ${ei.waveFt.toFixed(1)}ft${ei.wavePeriodS != null ? ` @ ${ei.wavePeriodS}s` : ""}` : "Wave n/a",
    ei.windKt != null ? `Wind ${Math.round(ei.windKt)}kt` : "Wind n/a",
    ei.clarityLabel ? `Clarity ${ei.clarityLabel}` : "Clarity n/a",
  ];
  // Priority 16 — satellite clarity, shown ONLY when a fresh reading
  // actually exists (commonly won't, given cloud cover + nearshore
  // limitations) — a real, labeled supplement, not a replacement for
  // the estimate above.
  if (ei.satelliteClarity) {
    parts.push(`Satellite ${ei.satelliteClarity.label} (${formatSatelliteAge(ei.satelliteClarity.ageHours)})`);
  }
  return parts.join(" · ");
}
...
function BestBeachComparatorScreen({ onSelectBeach }) {
  ...
  // Cleanest-water ranking — species-independent (clarity never depends on
  // species-specific scoring, only tide/wave/wind/precip), so rather than
  // running its OWN separate 24-beach prediction pass with a fixed anchor
  // species, this now derives directly from the species ranking above —
  // same underlying environmentalInputs, already computed, just re-sorted
  // by clarity instead of by score. Cuts this screen's total
  // predictAsOfTimestamp calls in half (24 instead of 48) on every hour
  // change, rather than relying on caching alone to make the duplicate
  // pass cheap.
  const clarityRanking = useMemo(() => {
    if (ranking.loading) return { loading: true, beaches: [], unavailableBeaches: [] };
    const withClarity = ranking.beaches.filter((b) => b.environmentalInputs?.clarityScore != null);
    const withoutClarity = ranking.beaches.filter((b) => b.environmentalInputs?.clarityScore == null);
    const ranked = [...withClarity].sort((a, b) => b.environmentalInputs.clarityScore - a.environmentalInputs.clarityScore);
    return {
      loading: false,
      beaches: ranked.map((b) => ({
            ...
            <div onClick={() => onSelectBeach(winner.beachId)} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 14, marginTop: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: "#7590A0", fontSize: 11, marginTop: 4 }}>
                  {formatConditionsSummary(winner.environmentalInputs)}
                </div>
              </div>
            </div>
            {runnerUp && (
              ...
              <div key={b.beachId} style={{ marginBottom: 4, borderRadius: 8, background: "#0E1B26", border: "1px solid #1F3444", overflow: "hidden" }}>
                ...
                {isExpanded && (
                  <div style={{ padding: "0 10px 10px" }}>
                    ...
                    <div style={{ color: "#8AA6B8", fontSize: 11, marginBottom: 6 }}>
                      {formatConditionsSummary(b.environmentalInputs)}
                    </div>
...
function ArchitectureScreen() {
  ...
  // Priority 15 Stage 3 — fetched automatically on load (unlike the
  // Calibration Check below, which is a manual, potentially-slow client
  // replay), since this is just reading numbers the cron already computed
  // server-side — cheap, no reason to gate it behind a button.
  const [forecastAccuracy, setForecastAccuracy] = useState({ loading: true, data: null, error: null });
  useEffect(() => {
    let cancelled = false;
    fetchForecastVerificationMetrics()
      .then((data) => { if (!cancelled) setForecastAccuracy({ loading: false, data, error: null }); })
      .catch((err) => { if (!cancelled) setForecastAccuracy({ loading: false, data: null, error: err.message }); });
    return () => { cancelled = true; };
  }, []);
  ...
  const runCalibrationCheck = async () => {
    ...
    try {
      ...
      // Per-species — reuses this SAME already-fetched backtest array,
      // just filtered per species (no re-fetch, no extra predictions run).
      // Answers a genuinely different question than the global numbers
      // above: does "70+" mean the same real-world hit rate for every
      // species, or are some species' scoring formulas just more generous
      // regardless of what's actually happening in the water. [0,39] vs
      // [70,100] matches discA's ranges above, so species can be compared
      // directly against the same global baseline.
      const perSpecies = SPECIES.map((sp) => {
        const d = computeDiscrimination(backtest, [0, 39], [70, 100], { species: sp.id });
        return { speciesId: sp.id, speciesName: sp.name, n: backtest.filter((r) => r.speciesId === sp.id && r.predictedScore != null && !r.error).length, disc: d };
      });
      setCalibState({ status: "done", data: { totalReplayed: backtest.length, global, discA, discB, perSpecies }, error: null });
    } catch (err) {
  ...
  return (
    <div style={{ padding: "12px 14px 90px" }}>
      ...
      {/* ============ FORECAST ACCURACY (Priority 15 Stage 3) ============
          Real forecasts issued on a schedule, then checked against real
          historical observations once their target time has actually
          passed — not a client-side replay, and not the same question as
          the Calibration Check below. This asks "were the forecasts
          themselves (wave/wind/tide, days out) actually right," across
          every beach/species the cron tracks. */}
      <div style={{ marginTop: 16, background: "#0E1B26", border: "1px solid #17D9C455", borderRadius: 10, padding: 12 }}>
        <div style={{ color: "#17D9C4", fontSize: 11, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: 0.4, marginBottom: 6 }}>
          FORECAST ACCURACY
        </div>
        <div style={{ color: "#7590A0", fontSize: 11, lineHeight: 1.5, marginBottom: 10 }}>
          Mean absolute error (MAE) — how far off, on average — and bias (positive = model runs high, negative = runs low) between what was forecast and what actually happened, by how many hours out the forecast was made. Never adjusts the model; measurement only.
        </div>
        {forecastAccuracy.loading ? (
          <div style={{ color: "#5A7A8A", fontSize: 12 }}>Loading…</div>
        ) : forecastAccuracy.error ? (
          <div style={{ color: "#FF5D5D", fontSize: 11.5 }}>Couldn't load: {forecastAccuracy.error}</div>
        ) : !forecastAccuracy.data?.horizons?.length ? (
          <div style={{ color: "#5A7A8A", fontSize: 12 }}>No verified forecasts yet — the cron issues and checks these on its own schedule, so this fills in over the first few days automatically.</div>
        ) : (
          <div>
            <div style={{ color: "#5A7A8A", fontSize: 10.5, marginBottom: 8 }}>
              model {forecastAccuracy.data.modelVersion || "—"} · minimum sample for a reliable number here: {forecastAccuracy.data.minN}
            </div>
            {forecastAccuracy.data.horizons.map((h) => (
              <div key={h.horizon_hours} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: "1px solid #16232E", fontSize: 11.5 }}>
                <span style={{ color: "#8AA6B8" }}>+{h.horizon_hours}h <span style={{ color: "#4A6270" }}>({h.n})</span></span>
                {h.reliable ? (
                  <span style={{ color: "#DCE8EE" }}>
                    score MAE {h.mae_score} <span style={{ color: h.bias_score > 0 ? "#F5A623" : h.bias_score < 0 ? "#3E8FFF" : "#5A7A8A" }}>({h.bias_score > 0 ? "+" : ""}{h.bias_score} bias)</span>
                  </span>
                ) : (
                  <span style={{ color: "#5A7A8A" }}>insufficient data</span>
                )}
              </div>
            ))}
          </div>
      ...
      <div style={{ marginTop: 16, background: "#0E1B26", border: "1px solid #3E8FFF55", borderRadius: 10, padding: 12 }}>
        ...
        {calibState.status === "done" && (() => {
          const { totalReplayed, global, discA, discB, perSpecies } = calibState.data;
          if (global.error) {
          ...
          return (
            <div>
              ...
              {/* Per-species — does "70+" mean the same real-world hit rate
                  for every species, or are some species' formulas just more
                  generous regardless of what's actually in the water. Same
                  0-39 vs 70-100 ranges as discA above, so each species'
                  numbers compare directly against that global baseline. */}
              <div style={{ color: "#7590A0", fontSize: 10.5, marginTop: 10, marginBottom: 4 }}>
                PER-SPECIES <span style={{ color: "#4A6270" }}>(0-39 vs 70-100 hit rate — is "70+" fair across species?)</span>
              </div>
              {(perSpecies || []).filter((p) => p.n > 0).length === 0 ? (
                <div style={{ color: "#5A7A8A", fontSize: 11.5 }}>No species have any replayed predictions yet.</div>
              ) : (
                (perSpecies || []).filter((p) => p.n > 0).sort((a, b) => b.n - a.n).map((p) => (
                  <div key={p.speciesId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: "1px solid #16232E", fontSize: 11.5 }}>
                    <span style={{ color: "#8AA6B8" }}>{p.speciesName} <span style={{ color: "#4A6270" }}>({p.n})</span></span>
                    {p.disc.error ? (
                      <span style={{ color: "#5A7A8A" }}>{p.disc.error}</span>
                    ) : !p.disc.reliable ? (
                      <span style={{ color: "#5A7A8A" }}>insufficient data</span>
                    ) : (
                      <span style={{ color: "#DCE8EE" }}>
                        low {p.disc.lowRange.observedPositiveRate != null ? `${Math.round(p.disc.lowRange.observedPositiveRate * 100)}%` : "—"} vs high{" "}
                        <span style={{ color: "#17D9C4", fontWeight: 600 }}>{p.disc.highRange.observedPositiveRate != null ? `${Math.round(p.disc.highRange.observedPositiveRate * 100)}%` : "—"}</span>
                      </span>
                    )}
                  </div>
                ))
              )}
              <div style={{ color: "#4A6270", fontSize: 10.5, marginTop: 6, lineHeight: 1.4 }}>
                If scoring is fair across species, the "high" numbers above should land in a similar range for every species with enough data — not just be high for whichever species happens to score generously.
              </div>
            </div>
          );
        })()}
